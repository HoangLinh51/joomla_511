...
class Danhmucfront_Model_Vtvlnhomvitrivieclam
{
	...
	public function save($formData)
	{
		...
		if (isset($formData['id']) && $formData['id'] > 0) {
			...
			$query->update($db->quoteName('vtvl_nhomvtvl'))->set($fields)->where($conditions);
			$db->setQuery($query);
			$db->execute();
			return $formData['id'];
		} else {
			$query->insert($db->quoteName('vtvl_nhomvtvl'));
			$query->set($fields);
			$db->setQuery($query);
			$db->execute();
			return $db->insertid();
	...
	function get($id){
		return Core::loadAssoc('vtvl_nhomvtvl','*', 'id='.$id);
	}
	/**
