...
class Danhmucfront_Model_Vtvldieukienlamviec
{
	public function getDanhsach()
	{
		...
		$columns = array(
			...
			array('db' => 'a.type',						'dt' => 2),
			array('db' => 'a.required',						'dt' => 3),
			array('db' => 'a.status',						'dt' => 4),
			array('db' => 'a.orders',							'dt' => 5),
			array('db' => 'a.id', 'alias' => 'aa',					'dt' => 6),
		);
		$table = 'vtvl_dieukienlamviec AS a';
		$primaryKey = 'a.id';
		$join = '';
	...
	public function save($formData)
	{
		...
		$fields = array(
			...
			$db->quoteName('description') . ' = ' . $db->quote($formData['description']),
			$db->quoteName('status') . ' = ' . $db->quote($formData['status']),
			$db->quoteName('orders') . ' = ' . $db->quote($formData['orders']),
			$db->quoteName('required') . ' = ' . $db->quote($formData['required']),
		);
		if (isset($formData['id']) && $formData['id'] > 0) {
			$conditions = array(
				$db->quoteName('id') . '=' . $db->quote($formData['id'])
			);
			$query->update($db->quoteName('vtvl_dieukienlamviec'))->set($fields)->where($conditions);
			$db->setQuery($query);
			$db->execute();
			return $formData['id'];
		} else {
			$query->insert($db->quoteName('vtvl_dieukienlamviec'));
			$query->set($fields);
			$db->setQuery($query);
			$db->execute();
			return $db->insertid();
	...
	function get($id){
		return Core::loadAssoc('vtvl_dieukienlamviec','*', 'id='.$id);
	}
	function xoa($id)
	{
		$db = JFactory::getDbo();
		...
		$query->delete('vtvl_dieukienlamviec');
