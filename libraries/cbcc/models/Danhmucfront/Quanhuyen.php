...
class Danhmucfront_Model_Quanhuyen
{
	public function getDanhsach()
	{
		...
		$columns = array(
			...
			array('db' => 'a.cadc_code',			'dt' => $i++),
			array('db' => (($i4223==1 && in_array('dist_code', $t4223))?'a.code_bnv':'a.code'), 'alias' => 'code_bnv','dt' => $i++),
			array('db' => 'a.status',						'dt' => $i++),
			array('db' => 'a.code', 'alias' => 'aa',		'dt' => $i++),
		);
		$table = 'dist_code AS a';
		$primaryKey = 'a.code';
		$join = '';
		$where[] = 'a.daxoa=0';
		...
		if ($flt_tinhthanh != null && count($flt_tinhthanh) > 0 && is_array($flt_tinhthanh))
			$where[] = 'a.cadc_code IN (' . implode(',', $flt_tinhthanh) . ')';

		$where = implode(' AND ', $where);
		$datatables = Core::model('Core/Datatables');
		return $datatables->simple($_POST, $table, $primaryKey, $columns, $join, $where.' ORDER BY cadc_code asc, muctuongduong is null asc, name is null asc');
	}
	public function save($formData)
	{
		...
		$fields = array(
			...
			$db->quoteName('cadc_code') . ' = ' . $db->quote($formData['cadc_code']),
		);
		$i4223 = Core::config('sync4223/is_use');
		$t4223 = explode(',' ,Core::config('sync4223/tbl'));
		if($i4223==1 && in_array('dist_code', $t4223)){
		...
		if (isset($formData['code']) && $formData['code']>0) {
			...
			$query->update($db->quoteName('dist_code'))->set($fields)->where($conditions);
			$db->setQuery($query);
			$db->execute();
			...
			$max = Core::loadResult('dist_code','max(code)', 'cadc_code='.$db->quote($formData['cadc_code']));
			if($max==0) $new_code= $formData['cadc_code'].'01';
			else $new_code= $formData['cadc_code'].str_replace($formData['cadc_code'],'',$max)+1;
			array_push($fields, $db->quoteName('code') . ' = '.$db->quote($new_code));
			$query->insert($db->quoteName('dist_code'));
			$query->set($fields);
			$db->setQuery($query);
			$db->execute();
			return $db->insertid();
	...
	function get($id){
		$db = JFactory::getDbo();
		return Core::loadAssoc('dist_code','*', 'code='.$db->quote($id));
	}
	/**
