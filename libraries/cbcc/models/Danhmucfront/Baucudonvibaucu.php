...
class Danhmucfront_Model_Baucudonvibaucu
{
	public function getDanhsach()
	{
		$array['flt_ten'] = JRequest::getVar('flt_ten', null);
		// $array['flt_trangthai'] = JRequest::getVar('flt_trangthai', null, 'string');
		$array['flt_diadiemhanhchinh_id'] = JRequest::getVar('flt_diadiemhanhchinh_id', '', 'string');
		$array['flt_capbaucu_id'] = JRequest::getVar('flt_capbaucu_id', '', 'string');
		...
		$columns = array(
			array('db' => 'a.id',					'dt' => $i++),
			array('db' => 'a.ten',					'dt' => $i++),
			array('db' => 'a.diaphuongbaucu_id',					'dt' => $i++),
			array('db' => 'a.capbaucu_id',					'dt' => $i++),
			array('db' => 'a.dotbaucu_id',					'dt' => $i++),
			array('db' => (($i4223 == 1 && in_array('baucu_donvibaucu', $t4223)) ? 'a.code_bnv' : 'a.id'), 'alias' => 'code_bnv', 'dt' => $i++),
			// array('db' => 'a.trangthai',						'dt' => $i++),
			array('db' => 'a.id', 'alias' => 'aa',					'dt' => $i++),
		);
		$table = 'baucu_donvibaucu AS a';
		$primaryKey = 'a.id';
		$join = '';
		$where[] = 'a.daxoa=0';
		...
		$flt_diadiemhanhchinh_id = self::stringToArrDb($array['flt_diadiemhanhchinh_id']);
		if ($flt_diadiemhanhchinh_id != null && count($flt_diadiemhanhchinh_id) > 0 && is_array($flt_diadiemhanhchinh_id))
			$where[] = 'a.diaphuongbaucu_id IN (' . implode(',', $flt_diadiemhanhchinh_id) . ')';
		$flt_capbaucu_id = self::stringToArrDb($array['flt_capbaucu_id']);
		if ($flt_capbaucu_id != null && count($flt_capbaucu_id) > 0 && is_array($flt_capbaucu_id))
			$where[] = 'a.capbaucu_id IN (' . implode(',', $flt_capbaucu_id) . ')';
	...
	public function save($formData)
	{
		...
		$i4223 = Core::config('sync4223/is_use');
		$t4223 = explode(',', Core::config('sync4223/tbl'));
		if ($i4223 == 1 && in_array('baucu_donvibaucu', $t4223)) {
		...
		if (isset($formData['id']) && $formData['id'] > 0) {
			...
			$query->update($db->quoteName('baucu_donvibaucu'))->set($fields)->where($conditions);
			$db->setQuery($query);
			$db->execute();
			return $formData['id'];
		} else {
			...
			$query->insert($db->quoteName('baucu_donvibaucu'));
			$query->set($fields);
			$db->setQuery($query);
			$db->execute();
			return $db->insertid();
	...
	function luuFkDiadiem($donvibaucu_id, $diadiem_id)
	{
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$fields = array(
			$db->quoteName('donvibaucu_id') . ' = ' . $db->quote($donvibaucu_id),
			$db->quoteName('diadiem_id') . ' = ' . $db->quote($diadiem_id),
		);
		array_push($fields, $db->quoteName('daxoa') . ' = 0');
		array_push($fields, $db->quoteName('ngaytao') . ' = now()');
		array_push($fields, $db->quoteName('nguoitao_id') . ' = ' . JFactory::getUser()->id);
		$query->insert($db->quoteName('baucu_donvibaucu2diadiem'));
		$query->set($fields);
		$db->setQuery($query);
		$db->execute();
		return $db->insertid();
	}
	function luuFkLoaiphieubau($donvibaucu_id, $loaiphieubau_id)
	{
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$fields = array(
			$db->quoteName('donvibaucu_id') . ' = ' . $db->quote($donvibaucu_id),
			$db->quoteName('loaiphieubau_id') . ' = ' . $db->quote($loaiphieubau_id),
		);
		array_push($fields, $db->quoteName('daxoa') . ' = 0');
		array_push($fields, $db->quoteName('ngaytao') . ' = now()');
		array_push($fields, $db->quoteName('nguoitao_id') . ' = ' . JFactory::getUser()->id);
		$query->insert($db->quoteName('baucu_donvibaucu2loaiphieubau'));
		$query->set($fields);
		$db->setQuery($query);
		$db->execute();
		return $db->insertid();
	}
	function luuFkNguoiungcu($donvibaucu_id, $nguoiungcu_id)
	{
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$fields = array(
			$db->quoteName('donvibaucu_id') . ' = ' . $db->quote($donvibaucu_id),
			$db->quoteName('nguoiungcu_id') . ' = ' . $db->quote($nguoiungcu_id),
		);
		array_push($fields, $db->quoteName('daxoa') . ' = 0');
		array_push($fields, $db->quoteName('ngaytao') . ' = now()');
		array_push($fields, $db->quoteName('nguoitao_id') . ' = ' . JFactory::getUser()->id);
		$query->insert($db->quoteName('baucu_donvibaucu2nguoiungcu'));
		$query->set($fields);
		$db->setQuery($query);
		$db->execute();
		return $db->insertid();
	}
	function luuFkTobaucu($p1, $p2)
	{
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$fields = array(
			$db->quoteName('donvibaucu_id') . ' = ' . $db->quote($p1),
			$db->quoteName('tobaucu_id') . ' = ' . $db->quote($p2),
		);
		array_push($fields, $db->quoteName('daxoa') . ' = 0');
		array_push($fields, $db->quoteName('ngaytao') . ' = now()');
		array_push($fields, $db->quoteName('nguoitao_id') . ' = ' . JFactory::getUser()->id);
		$query->insert($db->quoteName('baucu_donvibaucu2tobaucu'));
		$query->set($fields);
		$db->setQuery($query);
		$db->execute();
		return $db->insertid();
	...
	{
		$db = JFactory::getDbo();
		return Core::loadAssoc('baucu_donvibaucu', '*', 'id=' . $db->quote($id));
	}
	/**
