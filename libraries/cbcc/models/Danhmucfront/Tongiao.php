...
class Danhmucfront_Model_Tongiao
{
	public function getDanhsach()
	{
		...
		$columns = array(
			...
			array('db' => (($i4223==1 && in_array('rel_code', $t4223))?'a.code_bnv':'a.id'), 'alias' => 'code_bnv','dt' => $i++),
	...
	public function save($formData)
	{
		...
		$i4223 = Core::config('sync4223/is_use');
		$t4223 = explode(',' ,Core::config('sync4223/tbl'));
		if($i4223==1 && in_array('rel_code', $t4223)){
		...
		if (isset($formData['id']) && $formData['id']>0) {
			...
			$query->update($db->quoteName('rel_code'))->set($fields)->where($conditions);
			$db->setQuery($query);
			$db->execute();
			return $formData['id'];
		} else {
			array_push($fields, $db->quoteName('daxoa') . ' = 0');
			$query->insert($db->quoteName('rel_code'));
			$query->set($fields);
			$db->setQuery($query);
			$db->execute();
			return $db->insertid();
	...
	function get($id){
		$db = JFactory::getDbo();
		return Core::loadAssoc('rel_code','*', 'id='.$db->quote($id));
	}
	/**
