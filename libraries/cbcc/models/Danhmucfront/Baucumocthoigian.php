...
class Danhmucfront_Model_Baucumocthoigian
{
	public function getDanhsach()
	{
		...
		$columns = array(
			array('db' => 'a.id',					'dt' => $i++),
			array('db' => 'a.ten',					'dt' => $i++),
			array('db' => (($i4223==1 && in_array('baucu_mocthoigian', $t4223))?'a.code_bnv':'a.id'), 'alias' => 'code_bnv','dt' => $i++),
			array('db' => 'a.trangthai',						'dt' => $i++),
			array('db' => 'a.id', 'alias' => 'aa',					'dt' => $i++),
		);
		$table = 'baucu_mocthoigian AS a';
		$primaryKey = 'a.id';
		$join = '';
		$where[] = 'a.daxoa=0';
	...
	public function save($formData)
	{
		...
		$i4223 = Core::config('sync4223/is_use');
		$t4223 = explode(',' ,Core::config('sync4223/tbl'));
		if($i4223==1 && in_array('baucu_mocthoigian', $t4223)){
		...
		if (isset($formData['id']) && $formData['id']>0) {
			...
			$query->update($db->quoteName('baucu_mocthoigian'))->set($fields)->where($conditions);
			$db->setQuery($query);
			$db->execute();
			return $formData['id'];
		} else {
			...
			$query->insert($db->quoteName('baucu_mocthoigian'));
			$query->set($fields);
			$db->setQuery($query);
			$db->execute();
			return $db->insertid();
	...
	function xoa($id)
	{
		...
		// $query->delete('baucu_mocthoigian');
	...
	function get($id){
		$db = JFactory::getDbo();
		return Core::loadAssoc('baucu_mocthoigian','*', 'id='.$db->quote($id));
	}
	/**
