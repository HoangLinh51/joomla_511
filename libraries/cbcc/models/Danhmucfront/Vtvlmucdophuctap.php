...
class Danhmucfront_Model_Vtvlmucdophuctap
{
	...
	public function save($formData)
	{
		...
		if (isset($formData['id']) && $formData['id'] > 0) {
			...
			$query->update($db->quoteName('vtvl_mucdophuctap'))->set($fields)->where($conditions);
			$db->setQuery($query);
			$db->execute();
			return $formData['id'];
		} else {
			$query->insert($db->quoteName('vtvl_mucdophuctap'));
			$query->set($fields);
			$db->setQuery($query);
			$db->execute();
			return $db->insertid();
	...
	function xoa($id)
	{
		...
		$query->delete('vtvl_mucdophuctap');
	...
	function get($id){
		return Core::loadAssoc('vtvl_mucdophuctap','*', 'id='.$id);
	}
	/**
