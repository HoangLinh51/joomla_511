...
class Danhmucfront_Model_Vtvlmucdothamgia
{
	...
	public function save($formData)
	{
		...
		if (isset($formData['id']) && $formData['id'] > 0) {
			...
			$query->update($db->quoteName('vtvl_mucdothamgia'))->set($fields)->where($conditions);
			$db->setQuery($query);
			$db->execute();
			return $formData['id'];
		} else {
			$query->insert($db->quoteName('vtvl_mucdothamgia'));
			$query->set($fields);
			$db->setQuery($query);
			$db->execute();
			return $db->insertid();
	...
	function xoa($id)
	{
		...
		$query->delete('vtvl_mucdothamgia');
	...
	function get($id){
		return Core::loadAssoc('vtvl_mucdothamgia','*', 'id='.$id);
	}
	/**
