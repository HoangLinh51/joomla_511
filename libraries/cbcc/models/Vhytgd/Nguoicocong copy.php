  ...
  public function saveNguoicocong($formData)
{
    ...
    for ($ud = 0; $ud < $n_uudai; $ud++) {
        ...
        // Kiểm tra loaidungcu_id
        if ($data_uudai[$ud]['loaidungcu_id'] === 'undefined') {
            error_log("Bỏ qua bản ghi uudai2nguoicocong[$ud] vì loaidungcu_id không hợp lệ: " . print_r($data_uudai[$ud], true));
            continue;
        }
    ...
    public function getDanhSachNguoiCoCong($params = array(), $startFrom = 0, $perPage = 20)
    {
        ...
        $query->select([
            ...
            'a.n_cccd',
            'b.trangthai_id',
            'COALESCE(SUM(uudai.trocap), 0) AS tong_uudai' // Tính tổng trợ cấp từ bảng ưu đãi
        ]);
        ...
        $query->from('vhxhytgd_nguoicocong AS a')
            ...
            GROUP BY 
                nguoicocong_id
        ) AS uudai ON b.id = uudai.nguoicocong_id') // JOIN bảng ưu đãi
    ...
    public function CountDanhSachNCC($params = array())
    {
        ...
        $query->leftJoin('vhxhytgd_uudai2nguoicocong AS c ON b.id = c.nguoicocong_id');
    ...
    public function getEditNCC($ncc_id)
    {
        ...
        $query->innerJoin('dmdungcu AS dc ON dc.id = c.loaidungcu_id');
