<?php 
	...
	class Danhmuchethong_Model_Nguontuyendung extends JModelLegacy{
		public function findnguontuyendungbyname($tk_tennguontuyendung){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query->select('id,name,status,daxoa');
			$query->from('sour_rec_code');
			$query->where('daxoa!=1');
			$query->where('name LIKE '.$db->quote('%'.$tk_tennguontuyendung.'%'));
			$query->order('id asc');
			$db->setQuery($query);
			return $db->loadAssocList();
		}
		public function addnguontuyendung($form){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$columns= ('name,status,daxoa');
			$values = array($db->quote($form['name']),$db->quote($form['status']),0);
			$query->insert('sour_rec_code');
			$query->columns($columns);
			$query->values(implode(',',$values));
			$db->setQuery($query);
			return $db->query();
		}
		public function findnguontuyendung($id){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
		...
		public function updatenguontuyendung($form){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$field = array($db->quotename('name').'='.$db->quote($form['name']),
							$db->quotename('status').'='.$db->quote($form['status']));
			$condition = 'id='.$db->quote($form['id']);
			$query->update('sour_rec_code')->set($field)->where($condition);
			$db->setQuery($query);
			return $db->query();
		}
		public function xoanguontuyendung($id){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$condition = 'id='.$db->quote($id['id']);
			$query->update('sour_rec_code')->set('daxoa=1')->where($condition);
			$db->setQuery($query);
			return $db->query();
		}
		public function xoanhieunguontuyendung($id){
			$db = JFactory::getDbo();
			for($i=0;$i<count($id['id']);$i++){
				$query = $db->getQuery(true);
				$query->update('sour_rec_code')->set('daxoa=1');
		...
		public function xuat_excelnguontuyendung(){
			require 'components/com_danhmuc/classes/PHPExcel.php';        
	        $PHPExcel = new PHPExcel();
	        $PHPExcel->setActiveSheetIndex(0);
	        $PHPExcel->getActiveSheet()->mergeCells('B1:D1');
	        $PHPExcel->getActiveSheet()->setCellValue('B1','Danh sách nguồn tuyển dụng');
	        ...
	        $PHPExcel->getActiveSheet()->setCellValue('C3','Tên nguồn tuyển dụng');
	        ...
	        $timkiem_nguontuyendung = JRequest::getVar('ten');
	        $model = Core::model('Danhmuchethong/Nguontuyendung');
	        $ds_nguontuyendung = $model->findnguontuyendungbyname($timkiem_nguontuyendung);

	        $rowNumber = 4;
	        foreach($ds_nguontuyendung as $index=>$item){  
	        	if($item['status']==1){
