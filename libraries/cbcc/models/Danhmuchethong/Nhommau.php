...
class Danhmuchethong_Model_Nhommau extends BaseModel
{
	function findparty_posbyname($tk_nhommau)
	{
		$db = Factory::getDbo();
		$query = $db->getQuery(true);
		$query->select('code,name,level,status,daxoa');
		$query->from('danhmuc_nhommau');
		$query->where('name' . " LIKE '%$tk_nhommau%'");
		$query->where('daxoa!=1');
	...
	function updateparty_pos($form)
	{
		...
		$query->update('danhmuc_nhommau')->set($field)->where($condition);
		$db->setQuery($query);
		return $db->execute();
	}
