<?php 
	...
	class Danhmuchethong_Model_Rel extends JModelLegacy{
		function findrelbyname($tk_rel){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query->select('id,name,status,daxoa');
			$query->from('rel_code');
			$query->where('name'." LIKE ".$db->quote('%'.$tk_rel.'%'));
			$query->where('daxoa!=1');
			$query->order('id asc');
			// echo $query;die;
			$db->setQuery($query);
		...
		function addrel($form){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$columns = ('name,status,daxoa');
			$values = array($db->quote($form['name']),$db->quote($form['status']),0);
			$query->insert('rel_code')->columns($columns)->values(implode(',',$values));
			$db->setQuery($query);
			return $db->query();
		}
		function xoarel($id){
		...
		function updaterel($form){
			...
			$query->update('rel_code')->set($field)->where($condition);
			$db->setQuery($query);
			return $db->query();
		}
		function xoanhieurel($id){
			$db = JFactory::getDbo();
		...
		function xuat_excel(){
	        ...
	        $PHPExcel->getActiveSheet()->setCellValue('B1','Danh sách tôn giáo');
	        ...
	        $PHPExcel->getActiveSheet()->setCellValue('C3','Tên tôn giáo');
	        ...
	        $timkiem_rel = JRequest::getVar('ten');
	        $model = Core::model('Danhmuchethong/Rel');
	        $ds_rel = $model->findrelbyname($timkiem_rel);

	        $rowNumber = 4;
	        foreach($ds_rel as $index=>$item){  
