<?php 
	...
	class Danhmuchethong_Model_Awa extends JModelLegacy{
		function findawabyname($tk_awa){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query->select('id,name,status,daxoa');
			$query->from('awa_code');
			$query->where('name LIKE '.$db->quote('%'.$tk_awa.'%'));
			$query->where('daxoa!=1');
			$query->order('id asc');
			// echo $query;die;
			$db->setQuery($query);
		...
		function addawa($form){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$columns = ('name,status,daxoa');
			$values = array($db->quote($form['name']),$db->quote($form['status']),0);
			$query->insert('awa_code')->columns($columns)->values(implode(',',$values));
			$db->setQuery($query);
			return $db->query();
		}
		function xoaawa($id){
		...
		function updateawa($form){
			...
			$query->update('awa_code')->set($field)->where($condition);
			$db->setQuery($query);
			return $db->query();
		}
		function xoanhieuawa($id){
			$db = JFactory::getDbo();
		...
		function xuat_excel(){
	        ...
	        $PHPExcel->getActiveSheet()->setCellValue('B1','Danh sách danh hiệu phong tặng');
	        ...
	        $PHPExcel->getActiveSheet()->setCellValue('C3','Tên danh hiệu phong tặng');
	        ...
	        $timkiem_awa = JRequest::getVar('ten');
	        $model = Core::model('Danhmuchethong/Awa');
	        $ds_awa = $model->findawabyname($timkiem_awa);

	        $rowNumber = 4;
	        foreach($ds_awa as $index=>$item){  
