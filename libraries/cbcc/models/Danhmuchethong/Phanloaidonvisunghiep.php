<?php 
	...
	class Danhmuchethong_Model_Phanloaidonvisunghiep extends JModelLegacy{
		public function findphanloaidonvisunghiepbyname($tk_tenphanloaidonvisunghiep){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query->select('id,name,status,daxoa');
			$query->from('area_code');
			$query->where('daxoa!=1');
			$query->where('name LIKE '.$db->quote('%'.$tk_tenphanloaidonvisunghiep.'%'));
			$query->order('id asc');
			$db->setQuery($query);
			return $db->loadAssocList();
		}
		public function addphanloaidonvisunghiep($form){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$columns= ('name,status,daxoa');
			$values = array($db->quote($form['name']),$db->quote($form['status']),0);
			$query->insert('area_code');
			$query->columns($columns);
			$query->values(implode(',',$values));
			$db->setQuery($query);
			return $db->query();
		}
		public function findphanloaidonvisunghiep($id){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
		...
		public function updatephanloaidonvisunghiep($form){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$field = array($db->quotename('name').'='.$db->quote($form['name']),
							$db->quotename('status').'='.$db->quote($form['status']));
			$condition = 'id='.$db->quote($form['id']);
			$query->update('area_code')->set($field)->where($condition);
			$db->setQuery($query);
			return $db->query();
		}
		public function xoaphanloaidonvisunghiep($id){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$condition = 'id='.$db->quote($id['id']);
			$query->update('area_code')->set('daxoa=1')->where($condition);
			$db->setQuery($query);
			return $db->query();
		}
		public function xoanhieuphanloaidonvisunghiep($id){
			$db = JFactory::getDbo();
			for($i=0;$i<count($id['id']);$i++){
				$query = $db->getQuery(true);
				$query->update('area_code')->set('daxoa=1');
		...
		public function xuat_excelphanloaidonvisunghiep(){
			require 'components/com_danhmuc/classes/PHPExcel.php';        
	        $PHPExcel = new PHPExcel();
	        $PHPExcel->setActiveSheetIndex(0);
	        $PHPExcel->getActiveSheet()->mergeCells('B1:D1');
	        $PHPExcel->getActiveSheet()->setCellValue('B1','Danh sách phân loại đơn vị sự nghiệp');
	        ...
	        $PHPExcel->getActiveSheet()->setCellValue('C3','Tên phân loại đơn vị sự nghiệp');
	        ...
	        $timkiem_phanloaidonvisunghiep = JRequest::getVar('ten');
	        $model = Core::model('Danhmuchethong/Phanloaidonvisunghiep');
	        $ds_phanloaidonvisunghiep = $model->findphanloaidonvisunghiepbyname($timkiem_phanloaidonvisunghiep);

	        $rowNumber = 4;
	        foreach($ds_phanloaidonvisunghiep as $index=>$item){  
	        	if($item['status']==1){
