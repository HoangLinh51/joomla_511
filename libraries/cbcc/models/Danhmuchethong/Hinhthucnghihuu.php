<?php 
	...
	class Danhmuchethong_Model_Hinhthucnghihuu extends JModelLegacy{
		public function findhinhthucnghihuubyname($tk_tenhinhthucnghihuu){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$condition = array('name LIKE '.$db->quote('%'.$tk_tenhinhthucnghihuu.'%'),'daxoa!=1');
			$query->select('id,name,status,daxoa');
			$query->from('whois_ret_mgr');
			$query->where($condition);
			$query->order('id asc');
			$db->setQuery($query);
			return $db->loadAssocList();
		}
		public function addhinhthucnghihuu($form){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$columns = ('name,status,daxoa');
			$values = array($db->quote($form['name']),$db->quote($form['status']),0);
			$query->insert('whois_ret_mgr');
			$query->columns($columns);
			$query->values(implode(',',$values));
			$db->setQuery($query);
			return $db->query();
		}
		public function findhinhthucnghihuu($id){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
		...
		public function updatehinhthucnghihuu($form){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			...
			$query->update('whois_ret_mgr')->set($field)->where($condition);
			$db->setQuery($query);
			return $db->query();
		}
		public function deletehinhthucnghihuu($id){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query->update('whois_ret_mgr');
			$query->set('daxoa=1');
			$query->where('id='.$db->quote($id['id']));
			$db->setQuery($query);
			return $db->query();
		}
		public function deletemanyhinhthucnghihuu($id){
			$db = JFactory::getDbo();
		...
		public function xuat_excelhinhthucnghihuu(){
			require 'components/com_danhmuc/classes/PHPExcel.php';        
	        $PHPExcel = new PHPExcel();
	        $PHPExcel->setActiveSheetIndex(0);
	        $PHPExcel->getActiveSheet()->mergeCells('B1:D1');
	        $PHPExcel->getActiveSheet()->setCellValue('B1','Danh sách hình thức nghỉ hưu');
	        ...
	        $PHPExcel->getActiveSheet()->setCellValue('C3','Tên hình thức nghỉ hưu');
	        ...
	        $timkiem_hinhthucnghihuu = JRequest::getVar('ten');
	        $model = Core::model('Danhmuchethong/Hinhthucnghihuu');
	        $ds_hinhthucnghihuu = $model->findhinhthucnghihuubyname($timkiem_hinhthucnghihuu);

	        $rowNumber = 4;
	        foreach($ds_hinhthucnghihuu as $index=>$item){  
	        	if($item['status']==1){
