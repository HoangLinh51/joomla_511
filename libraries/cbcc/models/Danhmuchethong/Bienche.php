...
class Danhmuchethong_Model_Bienche extends BaseModel
{
	function findparty_posbyname($tk_bienche)
	{
		$db = Factory::getDbo();
		$query = $db->getQuery(true);
		$query->select('code,name,level,status,daxoa');
		$query->from('bienche');
		$query->where('name' . " LIKE '%$tk_bienche%'");
		$query->where('daxoa!=1');
	...
	function addparty_pos($form)
	{
		...
		$query->insert('bienche')->columns($columns)->values(implode(',', $values));
		$db->setQuery($query);
		return $db->execute();
	}
	...
	function updateparty_pos($form)
	{
		...
		$query->update('bienche')->set($field)->where($condition);
		$db->setQuery($query);
		return $db->execute();
	}
