<?php 
	...
	class Danhmuchethong_Model_Ran extends JModelLegacy{
		function findranbyname($tk_ran){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query->select('id,name,status,daxoa');
			$query->from('ran_code');
			$query->where('name'." LIKE ".$db->quote('%'.$tk_ran.'%'));
			$query->where('daxoa!=1');
			$query->order('id asc');
			// echo $query;die;
			$db->setQuery($query);
		...
		function addran($form){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$columns = ('name,status,daxoa');
			$values = array($db->quote($form['name']),$db->quote($form['status']),0);
			$query->insert('ran_code')->columns($columns)->values(implode(',',$values));
			$db->setQuery($query);
			return $db->query();
		}
		function xoaran($id){
		...
		function updateran($form){
			...
			$query->update('ran_code')->set($field)->where($condition);
			$db->setQuery($query);
			return $db->query();
		}
		function xoanhieuran($id){
			$db = JFactory::getDbo();
		...
		function xuat_excel(){
	        ...
	        $PHPExcel->getActiveSheet()->setCellValue('B1','Danh sách thành phần xuất thân');
	        ...
	        $PHPExcel->getActiveSheet()->setCellValue('C3','Tên thành phần xuất thân');
	        ...
	        $timkiem_ran = JRequest::getVar('ten');
	        $model = Core::model('Danhmuchethong/Ran');
	        $ds_ran = $model->findranbyname($timkiem_ran);

	        $rowNumber = 4;
	        foreach($ds_ran as $index=>$item){  
