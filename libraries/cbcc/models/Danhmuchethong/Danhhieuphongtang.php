...
class Danhmuchethong_Model_Danhhieuphongtang extends BaseModel
{
	function findparty_posbyname($tk_danhhieuphongtang)
	{
		$db = Factory::getDbo();
		$query = $db->getQuery(true);
		$query->select('code,name,level,status,daxoa');
		$query->from('danhmuc_danhhieuphongtang');
		$query->where('name' . " LIKE '%$tk_danhhieuphongtang%'");
		$query->where('daxoa!=1');
	...
	function addparty_pos($form)
	{
		...
		$query->insert('danhmuc_danhhieuphongtang')->columns($columns)->values(implode(',', $values));
		$db->setQuery($query);
		return $db->execute();
	}
	function xoaparty_pos($id)
	{
		...
		$query->update('danhmuc_danhhieuphongtang');
		$query->set('daxoa=1');
	...
	function updateparty_pos($form)
	{
		...
		$query->update('danhmuc_danhhieuphongtang')->set($field)->where($condition);
		$db->setQuery($query);
		return $db->execute();
	}
