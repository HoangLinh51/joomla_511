...
class Danhmuchethong_Model_Suckhoe extends BaseModel
{
	function findparty_posbyname($tk_thuongbinh)
	{
		$db = Factory::getDbo();
		$query = $db->getQuery(true);
		$query->select('code,name,level,status,daxoa');
		$query->from('danhmuc_tinhtrangsuckhoe');
		$query->where('name' . " LIKE '%$tk_thuongbinh%'");
		$query->where('daxoa!=1');
	...
	function xoaparty_pos($id)
	{
		...
		$query->update('danhmuc_tinhtrangsuckhoe');
		$query->set('daxoa=1');
	...
	function updateparty_pos($form)
	{
		...
		$query->update('danhmuc_tinhtrangsuckhoe')->set($field)->where($condition);
		$db->setQuery($query);
		return $db->execute();
	}
