...
class Danhmuchethong_Model_ThuongBinh extends BaseModel
{
	function findparty_posbyname($tk_thuongbinh)
	{
		...
		$query->from('danhmuc_hangthuongbinh');
	...
	function addparty_pos($form)
	{
		...
		$query->insert('danhmuc_hangthuongbinh')->columns($columns)->values(implode(',', $values));
		$db->setQuery($query);
		return $db->execute();
	}
	function xoaparty_pos($id)
	{
		...
		$query->update('danhmuc_hangthuongbinh');
		$query->set('daxoa=1');
	...
	function updateparty_pos($form)
	{
		...
		$query->update('danhmuc_hangthuongbinh')->set($field)->where($condition);
		$db->setQuery($query);
		return $db->execute();
	}
