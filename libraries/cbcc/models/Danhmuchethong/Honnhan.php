...
class Danhmuchethong_Model_Honnhan extends BaseModel
{
	function findparty_posbyname($tk_honnhan)
	{
		$db = Factory::getDbo();
		$query = $db->getQuery(true);
		$query->select('code,name,level,status,daxoa');
		$query->from('danhmuc_tinhtranghonnhan');
		$query->where('name' . " LIKE '%$tk_honnhan%'");
		$query->where('daxoa!=1');
	...
	function xoaparty_pos($id)
	{
		...
		$query->update('danhmuc_tinhtranghonnhan');
		$query->set('daxoa=1');
	...
	function updateparty_pos($form)
	{
		...
		$query->update('danhmuc_tinhtranghonnhan')->set($field)->where($condition);
		$db->setQuery($query);
		return $db->execute();
	}
