<?php 
	...
	class Danhmuchethong_Model_Cost extends JModelLegacy{
		function findcostbyname($tk_cost){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query->select('id,name,status,daxoa');
			$query->from('cost_code');
			$query->where('name LIKE '.$db->quote('%'.$tk_cost.'%'));
			$query->where('daxoa!=1');
			$query->order('id asc');
			// echo $query;die;
			$db->setQuery($query);
		...
		function addcost($form){
			...
			$query->insert('cost_code')->columns($columns)->values(implode(',',$values));
			$db->setQuery($query);
			return $db->query();
		}
		function xoacost($id){
		...
		function updatecost($form){
			...
			$query->update('cost_code')->set($field)->where($condition);
			$db->setQuery($query);
			return $db->query();
		}
		function xoanhieucost($id){
			$db = JFactory::getDbo();
		...
		function xuat_excel(){
	        ...
	        $PHPExcel->getActiveSheet()->setCellValue('B1','Danh sách nguồn kinh phí');
	        ...
	        $PHPExcel->getActiveSheet()->setCellValue('C3','Tên nguồn kinh phí');
	        ...
	        $timkiem_cost = JRequest::getVar('ten');
	        $model = Core::model('Danhmuchethong/Cost');
	        $ds_cost = $model->findcostbyname($timkiem_cost);

	        $rowNumber = 4;
	        foreach($ds_cost as $index=>$item){  
