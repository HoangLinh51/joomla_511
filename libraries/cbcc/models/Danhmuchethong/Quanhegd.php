...
class Danhmuchethong_Model_Quanhegd extends BaseModel
{
	function findparty_posbyname($tk_quanhegd)
	{
		$db = Factory::getDbo();
		$query = $db->getQuery(true);
		$query->select('code,name,level,status,daxoa');
		$query->from('danhmuc_quanhegiadinh');
		$query->where('name' . " LIKE '%$tk_quanhegd%'");
		$query->where('daxoa!=1');
	...
	function addparty_pos($form)
	{
		...
		$query->insert('danhmuc_quanhegiadinh')->columns($columns)->values(implode(',', $values));
		$db->setQuery($query);
		return $db->execute();
	}
	function xoaparty_pos($id)
	{
		...
		$query->update('danhmuc_quanhegiadinh');
		$query->set('daxoa=1');
	...
	function updateparty_pos($form)
	{
		...
		$query->update('danhmuc_quanhegiadinh')->set($field)->where($condition);
		$db->setQuery($query);
		return $db->execute();
	}
