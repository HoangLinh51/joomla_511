...
class Danhmuchethong_Model_Tongiao extends BaseModel
{
	function findparty_posbyname($tk_tongiao)
	{
		$db = Factory::getDbo();
		$query = $db->getQuery(true);
		$query->select('code,name,level,status,daxoa');
		$query->from('danhmuc_tongiao');
		$query->where('name' . " LIKE '%$tk_tongiao%'");
		$query->where('daxoa!=1');
	...
	function updateparty_pos($form)
	{
		...
		$query->update('danhmuc_tongiao')->set($field)->where($condition);
		$db->setQuery($query);
		return $db->execute();
	}
