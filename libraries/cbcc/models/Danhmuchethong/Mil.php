<?php 
	...
	class Danhmuchethong_Model_Mil extends JModelLegacy{
		function findmilbyname($tk_mil){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query->select('id,name,status,daxoa');
			$query->from('mil_code');
			$query->where('name'." LIKE ".$db->quote('%'.$tk_mil.'%'));
			$query->where('daxoa!=1');
			$query->order('id asc');
			// echo $query;die;
			$db->setQuery($query);
		...
		function addmil($form){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$columns = ('name,status,daxoa');
			$values = array($db->quote($form['name']),$db->quote($form['status']),0);
			$query->insert('mil_code')->columns($columns)->values(implode(',',$values));
			$db->setQuery($query);
			return $db->query();
		}
		function xoamil($id){
		...
		function updatemil($form){
			...
			$query->update('mil_code')->set($field)->where($condition);
			$db->setQuery($query);
			return $db->query();
		}
		function xoanhieumil($id){
			$db = JFactory::getDbo();
		...
		function xuat_excel(){
	        ...
	        $PHPExcel->getActiveSheet()->setCellValue('B1','Danh sách chức vụ lực lượng vũ trang');
	        ...
	        $PHPExcel->getActiveSheet()->setCellValue('C3','Tên chức vụ lực lượng vũ trang');
	        ...
	        $timkiem_mil = JRequest::getVar('ten');
	        $model = Core::model('Danhmuchethong/Mil');
	        $ds_mil = $model->findmilbyname($timkiem_mil);

	        $rowNumber = 4;
	        foreach($ds_mil as $index=>$item){  
