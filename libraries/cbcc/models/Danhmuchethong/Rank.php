<?php 
	...
	class Danhmuchethong_Model_Rank extends JModelLegacy{
		function findrankbyname($tk_rank){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query->select('id,name,status,daxoa');
			$query->from('rank_code');
			$query->where('name'." LIKE ".$db->quote('%'.$tk_rank.'%'));
			$query->where('daxoa!=1');
			$query->order('id asc');
			// echo $query;die;
			$db->setQuery($query);
		...
		function addrank($form){
			...
			$query->insert('rank_code')->columns($columns)->values(implode(',',$values));
			$db->setQuery($query);
			return $db->query();
		}
		function xoarank($id){
		...
		function updaterank($form){
			...
			$query->update('rank_code')->set($field)->where($condition);
			$db->setQuery($query);
			return $db->query();
		}
		function xoanhieurank($id){
			$db = JFactory::getDbo();
		...
		function xuat_excel(){
	        ...
	        $PHPExcel->getActiveSheet()->setCellValue('B1','Danh sách cấp bậc lực lượng vũ trang');
	        ...
	        $PHPExcel->getActiveSheet()->setCellValue('C3','Tên cấp bậc lực lượng vũ trang');
	        ...
	        $timkiem_rank = JRequest::getVar('ten');
	        $model = Core::model('Danhmuchethong/Rank');
	        $ds_rank = $model->findrankbyname($timkiem_rank);

	        $rowNumber = 4;
	        foreach($ds_rank as $index=>$item){  
