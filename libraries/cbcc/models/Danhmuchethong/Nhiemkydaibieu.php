<?php 
	...
	class Danhmuchethong_Model_Nhiemkydaibieu extends JModelLegacy{
		function findnhiemkydaibieubynameorcode($tk_nhiemkydaibieu){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query->select('id,name,status,daxoa');
			$query->from('elec_time');
			$query->where('name LIKE '.$db->quote('%'.$tk_nhiemkydaibieu.'%'));
			$query->where('daxoa!=1');
			$query->order('id asc');
			// echo $query;die;
			$db->setQuery($query);
		...
		function addnhiemkydaibieu($form){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$columns = ('name,status,daxoa');
			$values = array($db->quote($form['name']),$db->quote($form['status']),0);
			$query->insert('elec_time')->columns($columns)->values(implode(',',$values));
			$db->setQuery($query);
			return $db->query();
		}
		function deletenhiemkydaibieu($id){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$condition = 'id='.$db->quote($id);
			$query->update('elec_time');
			$query->set('daxoa=1');
		...
		function findnhiemkydaibieu($id){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$condition = 'id='.$db->quote($id);
			$query->select('id,name,status,daxoa');
		...
		function updatenhiemkydaibieu($form){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			...
			$query->update('elec_time')->set($field)->where($condition);
			// echo $query;die;
			$db->setQuery($query);
			return $db->query();
		}
		function deletemanynhiemkydaibieu($id){
			$db = JFactory::getDbo();
		...
		function xuat_excel(){
	        ...
	        $timkiem_nhiemkydaibieu = JRequest::getVar('ten');
	        // $model = Core::model('Danhmuckieubao/nhiemkydaibieu');
	        $ds_nhiemkydaibieu = $this->findnhiemkydaibieubyname($timkiem_nhiemkydaibieu);

	        $rowNumber = 4;
	        foreach($ds_nhiemkydaibieu as $index=>$item){  
