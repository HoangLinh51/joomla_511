<?php 
	...
	class Danhmuchethong_Model_Hea extends JModelLegacy{
		function findheabyname($tk_hea){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query->select('id,name,status,daxoa');
			$query->from('hea_code');
			$query->where('name'." LIKE ".$db->quote('%'.$tk_hea.'%'));
			$query->where('daxoa!=1');
			$query->order('id asc');
			// echo $query;die;
			$db->setQuery($query);
		...
		function addhea($form){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$columns = ('name,status,daxoa');
			$values = array($db->quote($form['name']),$db->quote($form['status']),0);
			$query->insert('hea_code')->columns($columns)->values(implode(',',$values));
			$db->setQuery($query);
			return $db->query();
		}
		function xoahea($id){
		...
		function updatehea($form){
			...
			$query->update('hea_code')->set($field)->where($condition);
			$db->setQuery($query);
			return $db->query();
		}
		function xoanhieuhea($id){
			$db = JFactory::getDbo();
		...
		function xuat_excel(){
	        ...
	        $PHPExcel->getActiveSheet()->setCellValue('B1','Danh sách sức khỏe');
	        ...
	        $PHPExcel->getActiveSheet()->setCellValue('C3','Tên sức khỏe');
	        ...
	        $timkiem_hea = JRequest::getVar('ten');
	        $model = Core::model('Danhmuchethong/Hea');
	        $ds_hea = $model->findheabyname($timkiem_hea);

	        $rowNumber = 4;
	        foreach($ds_hea as $index=>$item){  
