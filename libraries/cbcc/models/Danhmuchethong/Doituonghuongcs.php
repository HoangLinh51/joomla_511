...
class Danhmuchethong_Model_Doituonghuongcs extends BaseModel
{
	function findparty_posbyname($tk_doituonghuongcs)
	{
		$db = Factory::getDbo();
		$query = $db->getQuery(true);
		$query->select('code,name,level,status,daxoa');
		$query->from('danhmuc_doituonghuongchinhsach');
		$query->where('name' . " LIKE '%$tk_doituonghuongcs%'");
		$query->where('daxoa!=1');
	...
	function xoaparty_pos($id)
	{
		...
		$query->update('danhmuc_doituonghuongchinhsach');
		$query->set('daxoa=1');
	...
	function updateparty_pos($form)
	{
		...
		$query->update('danhmuc_doituonghuongchinhsach')->set($field)->where($condition);
		$db->setQuery($query);
		return $db->execute();
	}
