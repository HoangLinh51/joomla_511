<?php 
	...
	class Danhmuchethong_Model_Relative extends JModelLegacy{
		function findrelativebyname($tk_relative){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query->select('id,name,status,daxoa');
			$query->from('relative_code');
			$query->where('name'." LIKE ".$db->quote('%'.$tk_relative.'%'));
			$query->where('daxoa!=1');
		...
		function addrelative($form){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$columns = ('name,status,daxoa');
			$values = array($db->quote($form['name']),$db->quote($form['status']),0);
			$query->insert('relative_code')->columns($columns)->values(implode(',',$values));
			$db->setQuery($query);
			return $db->query();
		}
		function xoarelative($id){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$condition = 'id='.$db->quote($id['id']);
			$query->update('relative_code');
			$query->set('daxoa=1');
		...
		function updaterelative($form){
			...
			$query->update('relative_code')->set($field)->where($condition);
			$db->setQuery($query);
			return $db->query();
		}
		function xoanhieurelative($id){
			$db = JFactory::getDbo();
		...
		function xuat_excel(){
	        ...
	        $PHPExcel->getActiveSheet()->setCellValue('B1','Danh sách quan hệ gia đình');
	        ...
	        $PHPExcel->getActiveSheet()->setCellValue('C3','Tên quan hệ gia đình');
	        ...
	        $timkiem_relative = JRequest::getVar('ten');
	        $model = Core::model('Danhmuchethong/Relative');
	        $ds_relative = $model->findrelativebyname($timkiem_relative);

	        $rowNumber = 4;
	        foreach($ds_relative as $index=>$item){  
