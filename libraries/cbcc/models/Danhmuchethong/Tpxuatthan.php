...
class Danhmuchethong_Model_Tpxuatthan extends BaseModel
{
	function findparty_posbyname($tk_tpxuatthan)
	{
		$db = Factory::getDbo();
		$query = $db->getQuery(true);
		$query->select('code,name,level,status,daxoa');
		$query->from('danhmuc_thanhphanxuatthan');
		$query->where('name' . " LIKE '%$tk_tpxuatthan%'");
		$query->where('daxoa!=1');
	...
	function xoaparty_pos($id)
	{
		...
		$query->update('danhmuc_thanhphanxuatthan');
		$query->set('daxoa=1');
	...
	function updateparty_pos($form)
	{
		...
		$query->update('danhmuc_thanhphanxuatthan')->set($field)->where($condition);
		$db->setQuery($query);
		return $db->execute();
	}
