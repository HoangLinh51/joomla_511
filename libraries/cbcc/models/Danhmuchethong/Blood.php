<?php 
	...
	class Danhmuchethong_Model_Blood extends JModelLegacy{
		function findbloodbyname($tk_blood){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query->select('id,name,status,daxoa');
			$query->from('blood_code');
			$query->where('name LIKE '.$db->quote('%'.$tk_blood.'%'));
			$query->where('daxoa!=1');
			$query->order('id asc');
			// echo $query;die;
			$db->setQuery($query);
		...
		function updateblood($form){
			...
			$query->update('blood_code')->set($field)->where($condition);
			$db->setQuery($query);
			return $db->query();
		}
		function xoanhieublood($id){
			$db = JFactory::getDbo();
		...
		function xuat_excel(){
	        ...
	        $PHPExcel->getActiveSheet()->setCellValue('B1','Danh sách nhóm máu');
	        ...
	        $PHPExcel->getActiveSheet()->setCellValue('C3','Tên nhóm máu');
	        ...
	        $timkiem_blood = JRequest::getVar('ten');
	        $model = Core::model('Danhmuchethong/Blood');
	        $ds_blood = $model->findbloodbyname($timkiem_blood);

	        $rowNumber = 4;
	        foreach($ds_blood as $index=>$item){  
