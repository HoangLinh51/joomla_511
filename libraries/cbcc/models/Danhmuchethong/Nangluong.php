...
class Danhmuchethong_Model_Nangluong extends BaseModel
{
	function findparty_posbyname($tk_nangluong)
	{
		$db = Factory::getDbo();
		$query = $db->getQuery(true);
		$query->select('code,name,level,status,daxoa');
		$query->from('danhmuc_hinhthucnangluong');
		$query->where('name' . " LIKE '%$tk_nangluong%'");
		$query->where('daxoa!=1');
	...
	function xoaparty_pos($id)
	{
		...
		$query->update('danhmuc_hinhthucnangluong');
		$query->set('daxoa=1');
	...
	function updateparty_pos($form)
	{
		...
		$query->update('danhmuc_hinhthucnangluong')->set($field)->where($condition);
		$db->setQuery($query);
		return $db->execute();
	}
