<?php 
	...
	class Danhmuchethong_Model_Ability extends JModelLegacy{
		function findabilitybyname($tk_ability){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query->select('id,name,status,daxoa');
			$query->from('ability_code');
			$query->where('name LIKE '.$db->quote('%'.$tk_ability.'%'));
			$query->where('daxoa!=1');
			$query->order('id asc');
			// echo $query;die;
			$db->setQuery($query);
		...
		function addability($form){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$columns = ('name,status,daxoa');
			$values = array($db->quote($form['name']),$db->quote($form['status']),0);
			$query->insert('ability_code')->columns($columns)->values(implode(',',$values));
			$db->setQuery($query);
			return $db->query();
		}
		function xoaability($id){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$condition = 'id='.$db->quote($id['id']);
			$query->update('ability_code');
			$query->set('daxoa=1');
		...
		function updateability($form){
			...
			$query->update('ability_code')->set($field)->where($condition);
			$db->setQuery($query);
			return $db->query();
		}
		function xoanhieuability($id){
			$db = JFactory::getDbo();
		...
		function xuat_excel(){
	        ...
	        $PHPExcel->getActiveSheet()->setCellValue('B1','Danh sách năng lực sở trường');
	        ...
	        $PHPExcel->getActiveSheet()->setCellValue('C3','Tên năng lực sở trường');
	        ...
	        $timkiem_ability = JRequest::getVar('ten');
	        $model = Core::model('Danhmuckieubao/Ability');
	        $ds_ability = $model->findabilitybyname($timkiem_ability);

	        $rowNumber = 4;
	        foreach($ds_ability as $index=>$item){  
