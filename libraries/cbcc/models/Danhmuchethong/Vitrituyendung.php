<?php 
	...
	class Danhmuchethong_Model_Vitrituyendung extends JModelLegacy{
		public function findvitrituyendungbyname($tk_tenvitrituyendung){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$condition = array('name LIKE '.$db->quote('%'.$tk_tenvitrituyendung.'%'),'daxoa!=1');
			$query->select('id,name,status,daxoa');
			$query->from('form_rec_code');
			$query->where($condition);
			$db->setQuery($query);
			return $db->loadAssocList();
		}
		public function addvitrituyendung($form){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$columns = ('name,status,daxoa');
			$values = array($db->quote($form['name']),$db->quote($form['status']),0);
			$query->insert('form_rec_code');
			$query->columns($columns);
			$query->values(implode(',',$values));
			$db->setQuery($query);
			return $db->query();
		}
		public function findvitrituyendung($id){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
		...
		public function updatevitrituyendung($form){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			...
			$query->update('form_rec_code')->set($field)->where($condition);
			$db->setQuery($query);
			return $db->query();
		}
		public function deletevitrituyendung($id){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query->update('form_rec_code');
			$query->set('daxoa=1');
			$query->where('id='.$db->quote($id['id']));
			$db->setQuery($query);
			return $db->query();
		}
		public function deletemanyvitrituyendung($id){
			$db = JFactory::getDbo();
		...
		public function xuat_excelvitrituyendung(){
			require 'components/com_danhmuc/classes/PHPExcel.php';        
	        $PHPExcel = new PHPExcel();
	        $PHPExcel->setActiveSheetIndex(0);
	        $PHPExcel->getActiveSheet()->mergeCells('B1:D1');
	        $PHPExcel->getActiveSheet()->setCellValue('B1','Danh sách vị trí tuyển dụng');
	        ...
	        $PHPExcel->getActiveSheet()->setCellValue('C3','Tên vị trí tuyển dụng');
	        ...
	        $timkiem_vitrituyendung = JRequest::getVar('ten');
	        $model = Core::model('Danhmuchethong/Vitrituyendung');
	        $ds_vitrituyendung = $model->findvitrituyendungbyname($timkiem_vitrituyendung);

	        $rowNumber = 4;
	        foreach($ds_vitrituyendung as $index=>$item){ 
	        	if($item['status']==1){
