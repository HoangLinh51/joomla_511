...
class Danhmuchethong_Model_Gioitinh extends BaseModel
{
	function findparty_posbyname($tk_gioitinh)
	{
		$db = Factory::getDbo();
		$query = $db->getQuery(true);
		$query->select('code,name,level,status,daxoa');
		$query->from('danhmuc_gioitinh');
		$query->where('name' . " LIKE '%$tk_gioitinh%'");
		$query->where('daxoa!=1');
	...
	function updateparty_pos($form)
	{
		...
		$query->update('danhmuc_gioitinh')->set($field)->where($condition);
		$db->setQuery($query);
		return $db->execute();
	}
