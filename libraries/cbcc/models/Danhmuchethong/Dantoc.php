...
class Danhmuchethong_Model_Dantoc extends BaseModel
{
	function findparty_posbyname($tk_dantoc)
	{
		$db = Factory::getDbo();
		$query = $db->getQuery(true);
		$query->select('code,name,level,status,daxoa');
		$query->from('danhmuc_dantoc');
		$query->where('name' . " LIKE '%$tk_dantoc%'");
		$query->where('daxoa!=1');
	...
	function updateparty_pos($form)
	{
		...
		$query->update('danhmuc_dantoc')->set($field)->where($condition);
		$db->setQuery($query);
		return $db->execute();
	}
