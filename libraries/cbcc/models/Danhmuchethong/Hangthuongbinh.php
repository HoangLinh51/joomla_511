<?php 
	...
	class Danhmuchethong_Model_Hangthuongbinh extends JModelLegacy{
		function findhangthuongbinhbynameorcode($tk_hangthuongbinh){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$query->select('id,name,status,daxoa');
			$query->from('inv_code');
			$query->where('name LIKE '.$db->quote('%'.$tk_hangthuongbinh.'%'));
			$query->where('daxoa!=1');
			$query->order('id asc');
			// echo $query;die;
			$db->setQuery($query);
		...
		function addhangthuongbinh($form){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$columns = ('name,status,daxoa');
			$values = array($db->quote($form['name']),$db->quote($form['status']),0);
			$query->insert('inv_code')->columns($columns)->values(implode(',',$values));
			$db->setQuery($query);
			return $db->query();
		}
		function deletehangthuongbinh($id){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$condition = 'id='.$db->quote($id);
			$query->update('inv_code');
			$query->set('daxoa=1');
		...
		function findhangthuongbinh($id){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			$condition = 'id='.$db->quote($id);
			$query->select('id,name,status,daxoa');
		...
		function updatehangthuongbinh($form){
			$db = JFactory::getDbo();
			$query = $db->getQuery(true);
			...
			$query->update('inv_code')->set($field)->where($condition);
			// echo $query;die;
			$db->setQuery($query);
			return $db->query();
		}
		function deletemanyhangthuongbinh($id){
			$db = JFactory::getDbo();
		...
		function xuat_excel(){
	        ...
	        $timkiem_hangthuongbinh = JRequest::getVar('ten');
	        // $model = Core::model('Danhmuckieubao/Hangthuongbinh');
	        $ds_hangthuongbinh = $this->findhangthuongbinhbyname($timkiem_hangthuongbinh);

	        $rowNumber = 4;
	        foreach($ds_hangthuongbinh as $index=>$item){  
