<?php
class Vptk_Model_Danhmuc extends JModelLegacy{
    ...
    public function getDanhsachNguoidangky(){
        ...
        $query->innerJoin('danhmuc_khuvuc AS b1 ON a.thonto_id = b1.id');
        $query->innerJoin('danhmuc_khuvuc AS b2 ON a.phuongxa_id = b2.id');
        $query->innerJoin('danhmuc_khuvuc AS b3 ON a.quanhuyen_id = b3.id');
        $query->where('a.daxoa = 0');
    ...
    public function getTaikhoanById($taikhoan_id){
        ...
        $query->leftJoin('phanquyen_user2khuvuc AS c ON a.id = c.user_id');
    ...
    public function saveKhuvuc($formData){
        ...
        $query = $db->getQuery(true);
        if ((int) $data['id'] == 0) {
            $query->insert($db->quoteName('danhmuc_khuvuc'))->columns($data_insert_key)->values(implode(',', $data_insert_val));
        }else{
            $query->update($db->quoteName('danhmuc_khuvuc'));
            $query->set(implode(',', $data_update));
            $query->where(array($db->quoteName('id') . '=' . $db->quote($data['id'])));
        }
        $db->setQuery($query);
    ...
    public function saveTaikhoan($formData){
        ...
        if($formData['id'] == ''){
            ...
            $query = $db->getQuery(true);
            $query->delete('phanquyen_user2khuvuc')->where('user_id = '.$db->quote($data['id']));
            $db->setQuery($query);
            if(!$db->query()){
                return false;
            }
        ...
        $query = $db->getQuery(true);
        $query->insert($db->quoteName('phanquyen_user2khuvuc'));
    ...
    public function getTaikhoanDuocQuanly(){
        ...
        $query->innerJoin('phanquyen_user2khuvuc AS c ON a.id = c.user_id'.$condition);
    ...
    public function getNoidungTinnhanZalo($option = array()){
        ...
        $query->select('*');
        $query->from('dmh_thongtinnguoiquanly AS a');
        $query->innerJoin('phanquyen_user2khuvuc AS b ON a.user_id = b.user_id');
