...
class Danhmuc_Model_DanhmucThiduaKetqua extends JModelLegacy {

    public function create($formData) {
        $idMax = $this->getDataField('select max(id) id from danhmuc_thidua_ketqua');
        $idMax = (int) ($idMax) + 1;
        $db = JFactory::getDbo();
        $query = $db->getQuery(true);
 
        // Insert columns.
        $columns = array('id', 'ten',
            'trangthai', 'thidua_hinhthuc_id');
        // Insert values.
        $values = array($idMax, $db->quote($formData['ten']),
            $db->quote($formData['trangthai']), $db->quote($formData['thidua_hinhthuc_id']));
        // Prepare the insert query.
        $query
            ->insert($db->quoteName('danhmuc_thidua_ketqua'))
    ...
    public function update($formData) {
        $db = JFactory::getDbo();
        $query = 'UPDATE danhmuc_thidua_ketqua SET 
    			ten = "' . $formData['ten'] . '", trangthai = ' . $formData['trangthai'] .
                ', thidua_hinhthuc_id = ' . $formData['thidua_hinhthuc_id'] .
    ...
    public function read($id) {
        $table = Core::table('Danhmuc/DanhmucThiduaKetqua');
        if (!$table->load($id)) {
            return null;
        }
    ...
    public function delete($cid) {
        ...
        if (count($cid)) {
            ...
            for ($index = 0; $index < $count; $index = $index + 2) {
                $query = "DELETE FROM danhmuc_thidua_ketqua"
    ...
    public function getDataByID($id = null, $type = null) {
        $db = JFactory::getDbo();
        $query = 'select * from danhmuc_thidua_ketqua where id = ' . $id;
        $db->setQuery($query);
    ...
    function publish($cid = array(), $publish = 1) {
        ...
        if (count($cid)) {
            ...
            $count = count($cids);
            for ($index = 0; $index < $count; $index = $index + 2) {
                $query = 'UPDATE danhmuc_thidua_ketqua SET trangthai = ' . $publish .
                        ' WHERE id = ' . (int) $cids[$index];
                $db->setQuery($query);
                if (!$db->query()) {
                    $this->setError($this->_db->getErrorMsg());
