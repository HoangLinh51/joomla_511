 ...
 * Company: DNICT
 */
class Danhmuc_Model_Dmloaiphucap extends JModelLegacy{
	var $_data = null;
	var $_total = null;
	...
	public function _buildContentWhere(){
		...
		$filter_order = $this->mainframe->getUserStateFromRequest( $this->option.'filter_order', 'filter_order', 'tenloaiphucap', 'cmd' );
		...
		if ($search) {
			$where[] = 'LOWER(tenloaiphucap) LIKE '.$this->_db->Quote( '%'.$search.'%');
		}
		if($status != ''){
			$where[] = 'trangthaisudung = '.(int) $status;
		}
	...
	public function _buildContentOrderBy(){
		...
		$filter_order		= $this->mainframe->getUserStateFromRequest( $this->option.'filter_order','filter_order','','tenloaiphucap','cmd' );
		$filter_order_Dir	= $this->mainframe->getUserStateFromRequest( $this->option.'filter_order_Dir','filter_order_Dir',	'',	'word' );
		if (!in_array($filter_order, array('tenloaiphucap', 'id'))){
			$filter_order = 'tenloaiphucap';
		}
...
// 		var_dump($data);die;
		$db= JFactory::getDbo();
		$object = new stdClass();
		$object->tenloaiphucap = $data['tenloaiphucap'];
		$object->donvitinh = $data['donvitinh'];
		$object->cachtinh = $data['cachtinh'];
		$object->giatrimacdinh = $data['giatrimacdinh'];
		$object->colinhvuc = $data['colinhvuc'];
		$object->congaynangtieptheo = $data['congaynangtieptheo'];
		$object->theochucvu = $data['theochucvu'];
		$object->trangthaisudung = $data['trangthaisudung'];
		$object->key = $data['key'];
		$object->sonamtangphucap = $data['sonamtangphucap'];
		$object->muctangphucap = $data['muctangphucap'];
		if((int)$data['id'] == 0){
	...
	public function publish($table, $cid){
		...
		$ids = implode(",", $cid);
		$sql="UPDATE ".$table." SET trangthaisudung = 1 WHERE id IN ($ids)";
		$db->setQuery($sql);
		if (! $db->query()){
			return false;
		}
	...
	public function unpublish($table, $cid){
		...
		$ids = implode(",", $cid);
		$sql="UPDATE ".$table." SET trangthaisudung = 0 WHERE id IN ($ids)";
		$db->setQuery($sql);
		if (! $db->query()){
			return false;
		}
