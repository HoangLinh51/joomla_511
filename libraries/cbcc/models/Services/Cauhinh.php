...
class Services_Model_Cauhinh{
	public function save(){
		...
		$fields = array(
		    ...
		    $db->quoteName('webservices_id') . ' = ' . $db->quote($formData['webservices_id']),
		    $db->quoteName('loaidongbo_id') . ' = ' . $db->quote($formData['loaidongbo_id']),
		    $db->quoteName('mocthoigian') . ' = ' . $db->quote($formData['mocthoigian']),
		    $db->quoteName('status') . ' = ' . $db->quote($formData['status']),
		    $db->quoteName('dgcbcc_dotdanhgia_id') . ' = ' . $db->quote($formData['dgcbcc_dotdanhgia_id']),
		);
		if (isset($formData['id']) && $formData['id']>0){
			$conditions = array(
					$db->quoteName('id').'='.$db->quote($formData['id'])
			);
			$query->update($db->quoteName('dongbo_cauhinh'))->set($fields)->where($conditions);
		}
		else{
			$query->insert($db->quoteName('dongbo_cauhinh'));
			$query->set($fields);
		}
		// echo $query;die;
		$db->setQuery($query);
	...
	public function luulinkws(){
		// $post = JRequest::get('post');
		// $formData = $post['services'];
		$formData = JRequest::getVar('services');
		$db = JFactory::getDbo();
		$query = $db->getQuery(true);
		$fields = array(
		    $db->quoteName('donvi_id') . ' = ' . $db->quote($formData['donvi_id']),
		    $db->quoteName('donvi_name') . ' = ' . $db->quote(Core::loadResult('ins_dept','name','id='.(int)$formData['donvi_id'])),
		    $db->quoteName('linkservice') . ' = ' . $db->quote($formData['linkservice']),
		    $db->quoteName('trangthai') . ' = ' . $db->quote($formData['trangthai']),
		);
		if (isset($formData['id']) && $formData['id']>0){
			array_push($fields, $db->quoteName('nguoihieuchinh') . '=' . $db->quote(JFactory::getUser()->id));
			array_push($fields, $db->quoteName('ngayhieuchinh') . '= now()');
			$conditions = array(
					$db->quoteName('id').'='.$db->quote($formData['id'])
			);
			$query->update($db->quoteName('dgcbcc_services_tdoffice'))->set($fields)->where($conditions);
		}
		else{
			array_push($fields, $db->quoteName('nguoitao') . '=' . $db->quote(JFactory::getUser()->id));
			array_push($fields, $db->quoteName('ngaytao') . '= now()');
			$query->insert($db->quoteName('dgcbcc_services_tdoffice'));
			$query->set($fields);
		}
		// echo $query;die;
		$db->setQuery($query);
	...
	// public function luudomain(){
	// 	$formData = JRequest::get('services');
	// 	$db = JFactory::getDbo();
	// 	$query = $db->getQuery(true);
	// 	$fields = array(
	// 	    $db->quoteName('donvi_id') . ' = ' . $db->quote($formData['donvi_id']),
