...
class Services_Model_Webservices{
	public function save(){
		...
		$fields = array(
		    ...
		    $db->quoteName('link') . ' = ' . $db->quote($formData['link']),
		...
		if (isset($formData['id']) && $formData['id']>0){
			...
			$query->update($db->quoteName('danhmuc_webservices'))->set($fields)->where($conditions);
		}
		else{
			$query->insert($db->quoteName('danhmuc_webservices'));
			$query->set($fields);
		}
		// var_dump($query);die;
		$db->setQuery($query);
	...
	public function luuEmail(){
		...
		$query->update($db->quoteName('dgcbcc_taikhoan_tdoffice'))->set($fields)->where($conditions);
		// var_dump($query);die;
		$db->setQuery($query);
		return $db->query();
	}
