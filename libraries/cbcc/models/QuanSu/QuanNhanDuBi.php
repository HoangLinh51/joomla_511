...
class QuanSu_Model_QuanNhanDuBi extends BaseDatabaseModel
{
  ...
  public function getListQuanNhanDuBi($filters, $phuongxa)
  {
    $db = Factory::getDbo();
...
// exit;
  ...
  public function getThanNhanNgoai($dangKyId)
  {
    ...
    $query->select([
      ...
      ->from($db->quoteName('qs_thannhanquannhan', 'tn2'))
      ->leftJoin($db->quoteName('danhmuc_nghenghiep', 'nn2') . ' ON nn2.id = tn2.nghenghiep_id')
      ->where('tn2.daxoa = 0')
      ->where('tn2.quannhandubi_id = ' . (int)$dangKyId);
  ...
  public function getDanhSachXuatExcel($filters, $phuongxa)
  {
    ...
    $query->select([
      ...
      'a.n_dienthoai',
      'cb.tencapbac',
      'DATE_FORMAT(a.ngaynhapngu, "%d/%m/%Y") AS ngaynhapngu',
      'DATE_FORMAT(a.ngayxuatngu, "%d/%m/%Y") AS ngayxuatngu',
      'a.chucvu',
      'a.cnnvqs',
      'DATE_FORMAT(a.ngaychungnhan, "%d/%m/%Y") AS ngaychungnhan',
      'a.donvixuatngu',
      'a.ngachdubi',
    ...
    // ========== FROM ==========
    $query->from($db->quoteName('qs_quannhandubi', 'a'))
      ->leftJoin($db->quoteName('danhmuc_gioitinh', 'gt') . ' ON a.n_gioitinh_id = gt.id')
      ...
      ->leftJoin($db->quoteName('danhmuc_trangthaiquansu', 'ttqs') . ' ON ttqs.id = a.trangthaiquannhan_id')
      ->leftJoin($db->quoteName('danhmuc_capbac', 'cb') . ' ON cb.id = a.capbac_id')
      ->where('a.daxoa = 0');
