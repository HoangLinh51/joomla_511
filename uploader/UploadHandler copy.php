...
class UploadHandler
{
        ...
        if ($print_response) {
            ...
            if ($this->get_server_var('HTTP_CONTENT_RANGE')) {
                ...
                if ($files && is_array($files) && is_object($files[0]) && $files[0]->size) {
                    var_dump((int)$files[0]->size);exit;
                    $this->header('Range: 0-'.(
    ...
    protected function basename_default($filepath, $suffix = null) {
        $splited = preg_split('/\//', rtrim ($filepath, '/ '));
        return substr(basename('X'.$splited[count($splited)-1], $suffix), 1);
    }

    protected function basename($filepath, $suffix = '') {
        // Ensure $suffix is an empty string if it is null
        $suffix = $suffix ?? '';
        // Split the filepath into components
        $splited = preg_split('/\//', rtrim($filepath, '/ '));
        // Extract the base name with optional suffix
        $baseNameWithSuffix = basename('X' . $splited[count($splited) - 1], $suffix);
        // Return the base name without the leading 'X'
        return substr($baseNameWithSuffix, 1);
    }
