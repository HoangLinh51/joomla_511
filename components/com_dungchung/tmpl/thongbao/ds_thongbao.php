...
<script>
      ...
      if (item.vanban && Array.isArray(item.vanban)) {
        vanbanHtml = item.vanban.map(vb => {
          if (vb.type === 'application/pdf') {
            return `
            <a href="/index.php?option=com_dungchung&view=thongbao&format=raw&task=viewpdf&file=${vb.code}&folder=${vb.folder}" target="_blank">
              ${vb.filename}
            </a><br/>`;
          } else {
            return `
      ...
      return `
        <tr>
          ...
          <td class="text-center align-middle">
            ...
            <span class="btn btn-sm btn_xoa" style="font-size:18px;padding:10px;" data-thongbao="${item.id}" data-title="Xóa">
  ...
  async function loadData(page = 1, keyword = '', take = 20) {
    try {
      ...
      const response = await $.ajax({
        url: 'index.php?option=com_dungchung&controller=thongbao&task=getDanhSachThongBao',
        method: 'POST',
        data: {
          page,
          take,
  ...
  $(document).ready(function() {
    ...
    $('body').delegate('.btn_hieuchinh', 'click', function() {
      window.location.href = '/index.php?option=com_dungchung&view=thongbao&task=edit_thongbao&id=' + $(this).data('thongbao');
    });
    ...
    $('body').on('click', '.btn_xoa', function() {
      ...
      const url = "<?= Route::_('index.php?option=com_dungchung&task=thongbao.xoa_thongbao') ?>";
