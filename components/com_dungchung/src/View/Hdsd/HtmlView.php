...
class HtmlView extends BaseHtmlView
{
    public function display($tpl = null)
    {
        ...
        $modelHdsd = Core::model('DungChung/Base');
        $permissionAdmin = $modelHdsd->checkPermissionAdmin();
        $this->permissionAdmin = $permissionAdmin;
        if (($task === 'ADD_HDSD' || $task==="EDIT_HDSD") && $permissionAdmin === false) {
        ...
        switch ($task) {
            case 'DEFAULT':
            case 'DS_HDSD':
                $this->setLayout('ds_hdsd');
                $this->_initDefaultPage();
                break;
            case 'ADD_HDSD':
                $this->setLayout('edit_hdsd');
                break;
        }
    ...
    private function _initDefaultPage()
    {
        ...
        $listHdsd = $modelHdsd->getListHdsd();
        $this->listHdsd = $listHdsd;
    }

    public function _getEditHdsd($idHdsd)
    {
        $this->import();

        $modelHdsd = Core::model('DungChung/Hdsd');
        $itemHdsd = $modelHdsd->getDetailHdsd($idHdsd);
        if (empty($itemHdsd)) {
