...
<div class="modal-header">
    ...
    <h4 class="blue bigger" id="modal-title"><?php echo ($data['id'] > 0) ? 'Hiệu chỉnh' : 'Thêm mới' ?> Kết quả thi đua, đánh giá</h4>
</div>
<div class="modal-body">
    <form class="form-horizontal" id="form_ketquathidua" enctype="multipart/form-data">
        ...
        <div style="float:left;width:400px;">
            <div class="control-group">
                <label class="control-label" for="thidua_hinhthuc_id">Hình thức thi đua, đánh giá</label>
                <div class="controls">
                    <select name="frm[thidua_hinhthuc_id]" id="thidua_hinhthuc_id">
                        <?php for($i=0; $i<count($thidua_hinhthuc); $i++){ $row=$thidua_hinhthuc[$i];?>
                        <option value="<?php echo $row['id']?>" <?php if($row['id']==$data['thidua_hinhthuc_id']) echo 'selected'?>><?php echo $row['ten']?></option>
                        <?php }?>
                    </select>
                </div>
            </div>
...
<div class="modal-footer">
    <span class="btn btn-small btn-success" id="btn_ketquathidua_luu"><i class="icon-save"></i> Lưu</span>
...
<script>
    jQuery(document).ready(function($) {
        ...
        $('#btn_ketquathidua_luu').on('click', function() {
            $('#gritter-notice-wrapper').remove();
            if ($('#form_ketquathidua').valid() == true) {
                $.blockUI();
                $.ajax({
                    type: 'post',
                    url: 'index.php?option=com_danhmuc&controller=ketquathidua&task=luu_ketquathidua',
                    data: $('#form_ketquathidua').serialize(),
