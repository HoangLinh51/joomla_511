...
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			...
			<h3 class="smaller lighter blue no-margin"><?php echo mb_strlen($info['id'])>0?"Hiệu chỉnh":"Thêm mới"?> Giới tính</h3>
		</div>
		<div class="modal-body">
			<form class="form-horizontal" method="POST" name="frm_gioitinh" id="frm_gioitinh">
				<div class="row-fluid">
					<div class="control-group">
						<label class="control-label" for="id">Mã <span style="color:red;">*</span></label>
						<div class="controls">
							<input type="text" <?php if(mb_strlen($info['id'])>0) echo 'readonly';?> name="id" id="id" style="width:80%;" value="<?php echo $info['id']?>"/>
						</div>
					</div>
				</div>
				<div class="row-fluid">
		...
		<div class="modal-footer">
			...
			<span class="btn btn-sm btn-primary pull-right" id="btn_submit_gioitinh">
				<i class="ace-icon fa fa-times"></i>
				Lưu
			</span>
		</div>
...
jQuery(document).ready(function($){
	...
	$('#btn_submit_gioitinh').click(function(){
		$('.gritter-item-wrapper').remove();
		var flag = $('#frm_gioitinh').valid();
		if(flag == true){
			$.blockUI();
			formData = $('#frm_gioitinh').serialize();
			$.ajax({
				type: 'post',
				data: formData,
				url: 'index.php?option=com_danhmuc&controller=gioitinh&task=save&format=raw&',
