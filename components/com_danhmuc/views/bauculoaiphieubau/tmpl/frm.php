...
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			...
			<h3 class="smaller lighter blue no-margin"><?php echo ($info['id'])>0?"Hiệu chỉnh":"Thêm mới"?> Loại phiếu bầu</h3>
		</div>
		<div class="modal-body">
			<form class="form-horizontal" method="POST" name="frm_bauculoaiphieubau" id="frm_bauculoaiphieubau">
				...
				<div class="row-fluid">
					<div class="control-group">
						...
						<div class="controls">
						<input type="text" name="giatri" id="giatri" style="width:80%;" value="<?php echo $info['giatri']?>"/>
						</div>
					</div>
				</div>
				<div class="row-fluid">
		...
		<div class="modal-footer">
			...
			<span class="btn btn-sm btn-primary pull-right" id="btn_submit_bauculoaiphieubau">
				<i class="ace-icon fa fa-times"></i>
				Lưu
			</span>
		</div>
...
jQuery(document).ready(function($){
	...
	$('.dp').mask('99/99/9999');
	$('#frm_bauculoaiphieubau').validate({
	...
	$('#btn_submit_bauculoaiphieubau').click(function(){
		$('.gritter-item-wrapper').remove();
		var flag = $('#frm_bauculoaiphieubau').valid();
		if(flag == true){
			$.blockUI();
			formData = $('#frm_bauculoaiphieubau').serialize();
			$.ajax({
				type: 'post',
				data: formData,
				url: 'index.php?option=com_danhmuc&controller=bauculoaiphieubau&task=save&format=raw&',
