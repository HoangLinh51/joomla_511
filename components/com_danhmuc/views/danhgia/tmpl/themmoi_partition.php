...
<div class="modal-body overflow-visible">
	...
	<form class="form-horizontal" id="form_partition" enctype="multipart/form-data">
		<?php echo JHtml::_( 'form.token' ); ?>
		<div class="row-fluid">
			<div class="control-group">
				<label class="control-label span4">Table (<span style="color:red">*</span>)</label>
				<div class="controls span8">
					<input type="text" name="frm[table]" class="span12" value="<?php if($id>0) echo $partition['table']; ?>">
					<input type="hidden" name="frm[id]" value="<?php if($id>0) echo $partition['id']; ?>">
				</div>
			</div>
			<div class="control-group">
				<label class="control-label span4">Key Partition (<span style="color:red">*</span>)</label>
				<div class="controls span8">
					<input type="text" name="frm[key_partition]" class="span12" value="<?php if($id>0) echo $partition['key_partition']; ?>">
				</div>
			</div>
			<div class="control-group">
				<label class="control-label span4">Key Table (<span style="color:red">*</span>)</label>
				<div class="controls span8">
					<input type="text" name="frm[key_table]" class="span12" value="<?php if($id>0) echo $partition['key_table']; ?>">
				</div>
			</div>
			<div class="control-group">
				...
				<div class="controls span8">
					<label>
						<input type="checkbox" value="1" name="frm[status]" <?php if($id>0&&$partition['status']==1) echo 'checked'; ?>>
...
<div class="modal-footer">
	<span class="btn btn-small btn-success" id="btn_partition_luu">Lưu</span>
	<button class="btn btn-small btn-danger" data-dismiss="modal">Quay lại</button>
</div>
<script>
	jQuery(document).ready(function($){
		$('#form_partition').validate({
            rules:{
        ...
        $('#btn_partition_luu').on('click',function(){
        	if($('#form_partition').valid()==true){
        		$.blockUI();
        		$.ajax({
        			type: 'post',
        			url: 'index.php?option=com_danhmuc&controller=partition&task=luu_partition',
        			data: $('#form_partition').serialize(),
