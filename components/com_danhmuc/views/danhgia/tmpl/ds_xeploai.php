...
<div id="ds_xeploai">
	<h2 class="header">Xếp loại
		<div class="pull-right">
			<span class="btn btn-small btn-success" id="btn_themmoi_xeploai" href="#modal-form" data-toggle="modal">Thêm mới</span>
			<span class="btn btn-small btn-danger" id="btn_xoanhieu_xeploai">Xóa</span>
		</div>
	</h2>
...
<script>
	jQuery(document).ready(function($){
		$('#select_id_dotdanhgia').on('change',function(){
			...
			$('#table_xeploai').load('index.php?option=com_danhmuc&controller=danhgia&task=table_xeploai&format=raw&id_dotdanhgia='+id_dotdanhgia,function(){
				$.unblockUI();
			});
		});
		...
		$('#btn_themmoi_xeploai').on('click',function(){
			$('#modal-form').html('');
		...
		$('#btn_xoanhieu_xeploai').on('click',function(){
			var array_id = [];
			$('.array_id_xeploai:checked').each(function(){
			...
			if(array_id.length>0){
				if(confirm("Bạn có muốn xóa không?")){
					...
					$.ajax({
						type: 'post',
						url: 'index.php?option=com_danhmuc&controller=xeploai&task=xoanhieu_xeploai',
