<?php 
	...
	if($id>0){
		$dgcbcc_dotdanhgia = $this->dgcbcc_dotdanhgia;
		$date_dot = $model->ConvertFormatDateFromDb($dgcbcc_dotdanhgia['date_dot']);
		$ngaybatdau = $model->ConvertFormatDateFromDb($dgcbcc_dotdanhgia['ngaybatdau']);
		$ngaytudanhgia = $model->ConvertFormatDateFromDb($dgcbcc_dotdanhgia['ngaytudanhgia']);
		$ngayketthuc = $model->ConvertFormatDateFromDb($dgcbcc_dotdanhgia['ngayketthuc']);
	}
?>
...
<div class="modal-body overflow-visible">
	...
	<form class="form-horizontal" id="form_dgcbcc_dotdanhgia" enctype="multipart/form-data">
		...
		<div class="row-fluid">
			<div class="control-group">
				...
				<div class="controls span9">
					<select class="chzn-select span12" name="frm[id_botieuchi]">
						...
						<option value="<?php echo $ds_dgcbcc_botieuchi[$i]['id']; ?>" <?php if($id>0&&$dgcbcc_dotdanhgia['id_botieuchi']==$ds_dgcbcc_botieuchi[$i]['id']) echo 'selected'; ?>>
			...
			<div class="control-group">
				...
				<div class="controls span9">
					<input type="text" name="frm[name]" class="span12" value="<?php if($id>0) echo $dgcbcc_dotdanhgia['name']; ?>">
					<input type="hidden" name="frm[id]" value="<?php if($id>0) echo $dgcbcc_dotdanhgia['id']; ?>">
				</div>
			</div>
			<div class="control-group">
			...
			<div class="control-group">
				...
				<div class="controls span9">
					<input type="text" name="frm[time_dot]" class="span12" value="<?php if($id>0) echo $dgcbcc_dotdanhgia['time_dot']; ?>">
				</div>
			</div>
			<div class="control-group">
				...
				<div class="controls span9">
					<label>
						<input type="checkbox" name="frm[is_danhgianam]" value="1" <?php if($id>0&&$dgcbcc_dotdanhgia['is_danhgianam']==1) echo 'checked'; ?>>
			...
			<div class="control-group">
				...
				<div class="controls span9">
					<label>
						<input type="checkbox" name="frm[is_lock]" value="1" <?php if($id>0&&$dgcbcc_dotdanhgia['is_lock']==1) echo 'checked'; ?>>
			...
			<div class="control-group">
				...
				<div class="controls span9">
					<input type="text" name="frm[ghichu]" class="span12"value="<?php if($id>0) echo $dgcbcc_dotdanhgia['ghichu']; ?>">
				</div>
			</div>
		</div>
	</form>
...
<div class="modal-footer">
	<span class="btn btn-small btn-success" id="btn_dgcbcc_dotdanhgia_luu">Lưu</span>
...
<script>
	jQuery(document).ready(function($){
		...
		$('#form_dgcbcc_dotdanhgia').validate({
            rules:{
        ...
        $('#btn_dgcbcc_dotdanhgia_luu').on('click',function(){
        	if($('#form_dgcbcc_dotdanhgia').valid()==true){
        		$.blockUI();
        		$.ajax({
        			type: 'post',
        			url: 'index.php?option=com_danhmuc&controller=dgcbcc_dotdanhgia&task=luu_dgcbcc_dotdanhgia',
        			data: $('#form_dgcbcc_dotdanhgia').serialize(),
        			success:function(data){
        				if(data>0){
        					$('#modal-form').modal('hide');      					
        					loadNoticeBoardSuccess('Thông báo','Xử lý thành công');
        					$.unblockUI();
        					window.location.href='index.php?option=com_danhmuc&controller=danhgia&task=ds_dgcbcc_dotdanhgia';
        				}
        				else{
