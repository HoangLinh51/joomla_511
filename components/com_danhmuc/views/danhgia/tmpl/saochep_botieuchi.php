...
<div class="modal-body overflow-visible">
	...
	<form class="form-horizontal" id="form_saochep_botieuchi" enctype="multipart/form-data">
			...
			<div class="control-group">
				...
				<div class="controls span7">
					<select class="chzn-select span12" name="frm[botieuchicon]">
						...
						<option value="<?php echo $ds_botieuchi[$i]['id']; ?>"><?php echo $ds_botieuchi[$i]['name']; ?></option>
						<?php } ?>
					</select>
				</div>
			</div>
...
<div class="modal-footer">
	<span class="btn btn-small btn-success" id="btn_botieuchi_saochep">Lưu</span>
	<button class="btn btn-small btn-danger" data-dismiss="modal">Quay lại</button>
</div>
<script>
	jQuery(document).ready(function($){
		$('.chzn-select').chosen({"width":"100%","search_contains":true});
		$('#form_saochep_botieuchi').validate({
            rules:{
        ...
        $("#btn_botieuchi_saochep").on('click',function(){
        	if($('#form_saochep_botieuchi').valid()==true){
        		$.blockUI();
        		$.ajax({
        			type: 'post',
        			url: 'index.php?option=com_danhmuc&controller=botieuchi&task=saochep_botieuchi',
        			data: $('#form_saochep_botieuchi').serialize(),
