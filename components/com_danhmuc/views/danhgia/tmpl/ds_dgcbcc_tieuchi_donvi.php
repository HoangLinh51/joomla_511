<?php 
	$model = Core::model('Danhgia/DgcbccTieuchiDonvi');
	...
	jQuery(document).ready(function($){
		...
		$("#main-content-tree").jstree({
				...
				"ajax" : {
					...
					"data" : function (n) {
						return {
				     			"option" : "com_danhmuc",   
				     			"view" : "danhgia",                         
				     			"task" : "tree_dgcbcc_tieuchi_donvi",
				     			"format" : "raw",                            
				     			"root_id" :  root_id,
			...
			if(id>0){
				...
				$.ajax({
					type: 'post',
					url: 'index.php?option=com_danhmuc&controller=dgcbcctieuchidonvi&task=find_tieuchi_by_botieuchi',
					...
					success:function(data){
						if(data==''){
							...
							for(var i=0;i<data.length;i++){
								...
								xhtml += '<td style="vertical-align:middle" class="center"><span class="btn btn-mini btn-primary" id="btn_change_dgcbcc_tieuchi" data-id="'+data[i]['id_tieuchi']+'" href="#modal-form" data-toggle="modal">Thay đổi</span></td>';
								xhtml += '</tr>';
								stt += 1;
							}
		...
		$('body').delegate('#btn_change_dgcbcc_tieuchi','click',function(){
			var id = $(this).data('id');
			$('#modal-form').html('');
	    	$('#modal-form').load('<?php echo JURI::base(true);?>/index.php?option=com_danhmuc&controller=danhgia&task=change_dgcbcc_tieuchi_donvi&format=raw&id_tieuchi='+id, function(){
	    	});
		});
	});
</script>