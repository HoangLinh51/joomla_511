...
?>
<div id="ds_dgcbcc_dotdanhgia">
	<h2 class="header">
		Đợt đánh giá cán bộ công chức
		<div class="pull-right">
			<span class="btn btn-small btn-success" id="btn_themmoi_dgcbcc_dotdanhgia" href="#modal-form" data-toggle="modal">Thêm mới</span>
			<span class="btn btn-small btn-danger" id="btn_xoanhieu_dgcbcc_dotdanhgia">Xóa</span>
		</div>
	</h2>
	<div class="pull-right row-fluid">
		...
		<div class="span3">
			...
			<div class="span4">
				<select class="span12" id="dgcbcc_dotdanhgia_id">
				<?php 
			...
			<div class="span4">
				<select class="span12" id="dgcbcc_dotdanhgia_nam">
	...
	<div id="table_dgcbcc_dotdanhgia"></div>
</div>
<?php if($format==''){ ?>
<div id="content"></div>
...
<script>	
	jQuery(document).ready(function($){
		...
		var loadTable = function(){
			$.blockUI();
			var date_dot = $('#dgcbcc_dotdanhgia_id').val();
			$('#table_dgcbcc_dotdanhgia').load('index.php?option=com_danhmuc&controller=danhgia&task=table_dgcbcc_dotdanhgia&format=raw&date_dot='+date_dot,function(){
				$.unblockUI();
			});
		};
		loadTable();
		$('#dgcbcc_dotdanhgia_nam').on('change',function(){
			...
			$("#dgcbcc_dotdanhgia_id").html(xhtml);
			// $('#dgcbcc_dotdanhgia_id').change();
		});
		$('#dgcbcc_dotdanhgia_id').on('change',function(){
			setCookie('danhgia_id_dotdanhgia',$(this).val());
			loadTable();
		});
		$('#btn_themmoi_dgcbcc_dotdanhgia').on('click',function(){
			$('#modal-form').html('');
	    	$('#modal-form').load('<?php echo JURI::base(true);?>/index.php?option=com_danhmuc&controller=danhgia&task=themmoi_dgcbcc_dotdanhgia&format=raw', function(){
	    	});
		});
		$('#btn_xoanhieu_dgcbcc_dotdanhgia').on('click',function(){
			if(confirm('Bạn có muốn xóa không?')){
				$.blockUI();
				var array_id = [];
				$('.array_id_dgcbcc_dotdanhgia').each(function(){
					array_id.push($(this).val());
				});
				$.ajax({
					type: 'post',
					url: 'index.php?option=com_danhmuc&controller=dgcbcc_dotdanhgia&task=xoanhieu_dgcbcc_dotdanhgia',
					data: {id:array_id},
