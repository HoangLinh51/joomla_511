...
<div class="modal-body overflow-visible" style="overflow:scroll">
    <div id="modal-content" class="slim-scroll" data-height="350">
		<form class="form-horizontal" id="form_theonhiemvu" enctype="multipart/form-data">
					...
					<div id="content_thongtinnhiemvu" class="tab-pane in active">
						<div class="row-fluid control-group">
							...
							<div class="controls span9">
								<input type="text" class="span12" name="frm[name]" autocomplete="off" value="<?php if($id>0) echo $theonhiemvu['name']; ?>">
								<input type="hidden" name="frm[id]" value="<?php if($id>0) echo $theonhiemvu['id']; ?>">
							</div>
						</div>
						<div class="row-fluid control-group">
							<div class="span3 control-label">Mã nhiệm vụ (<span style="color:red">*</span>)</div>
							<div class="controls span9">
								<input type="text" class="span12" name="frm[code]" autocomplete="off" value="<?php if($id>0) echo $theonhiemvu['code']; ?>">
							</div>
						</div>
						<div class="row-fluid control-group">
							<div class="span3 control-label">Biểu mẫu</div>
							<div class="controls span9">
								<input type="text" class="span12" name="frm[template]" autocomplete="off" value="<?php if($id>0) echo $theonhiemvu['template']; ?>">
							</div>
						</div>
					</div>
...
<script>
	jQuery(document).ready(function($){
        ...
        $('#btn_dgcbcc_luu').on('click',function(){
        	if($('#form_theonhiemvu').valid()==true){
        		$.blockUI();
        		$.ajax({
        			type: 'post',
        			url: 'index.php?option=com_danhmuc&controller=theonhiemvu&task=luu_theonhiemvu',
        			data: $('#form_theonhiemvu').serialize(),
