...
?>
<div id="ds_dgcbcc_mucdophuctap_xeploai">
	<h2 class="header">Mức độ phức tạp
		<div class="pull-right">
			<span class="btn btn-small btn-success" id="btn_themmoi_dgcbcc_mucdophuctap" href="#modal-form" data-toggle="modal">Thêm mới</span>
			<span class="btn btn-small btn-danger" id="btn_xoanhieu_dgcbcc_mucdophuctap">Xóa</span>
		</div>
	</h2>
	...
	<div id="table_dgcbcc_mucdophuctap_xeploai"></div>
</div>
<?php if($format==''){ ?>
<div id="content"></div>
...
<script>
	jQuery(document).ready(function($){
		$('#select_id_dotdanhgia').on('change',function(){
			...
			$('#table_dgcbcc_mucdophuctap_xeploai').load('index.php?option=com_danhmuc&controller=danhgia&task=table_dgcbcc_mucdophuctap_xeploai&format=raw&id_dotdanhgia='+id_dotdanhgia,function(){
				$.unblockUI();
			});
		});
		...
		$('#btn_themmoi_dgcbcc_mucdophuctap').on('click',function(){
			$('#modal-form').html('');
		...
		$('#btn_xoanhieu_dgcbcc_mucdophuctap').on('click',function(){
			var array_id = [];
			$('.array_id_dgcbcc_mucdophuctap:checked').each(function(){
			...
			if(array_id.length>0){
				if(confirm("Bạn có muốn xóa không?")){
					...
					$.ajax({
						type: 'post',
						url: 'index.php?option=com_danhmuc&controller=dgcbcc_mucdophuctap&task=xoanhieu_dgcbcc_mucdophuctap',
