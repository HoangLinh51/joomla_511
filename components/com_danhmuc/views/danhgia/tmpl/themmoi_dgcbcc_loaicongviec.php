<?php 
	...
	if($id>0){
		$dgcbcc_loaicongviec = $this->dgcbcc_loaicongviec;
...
<div class="modal-body overflow-visible" style="overflow-y:scroll">
	...
	<form class="form-horizontal" id="form_dgcbcc_loaicongviec" enctype="multipart/form-data">
                    ...
                    <div class="row-fluid">
                        <div class="control-group">
                            ...
                            <div class="controls span8">
                                <input type="text" name="frm[name]" class="span12" autocomplete="off" value="<?php if($id>0) echo $dgcbcc_loaicongviec['name']; ?>">
                                <input type="hidden" name="frm[id]" class="span12" value="<?php if($id>0) echo $dgcbcc_loaicongviec['id']; ?>">
                            </div>
                        </div>
                        <div class="control-group">
                            ...
                            <div class="controls span8">
                                <label>
                                    <input type="checkbox" name="frm[status]" value="1" <?php if($id>0&&$dgcbcc_loaicongviec['status']==1) echo 'checked'; ?>>
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            ...
                            <?php 
                                ...
                                if($id>0&&count($ds_iddotdanhgia)>0){
                                    for($j=0;$j<count($ds_iddotdanhgia);$j++){
                                        if($ds_iddotdanhgia[$j]['id_dotdanhgia']==$ds_dgcbcc_dotdanhgia[$i]['id']){
                                            $check = 1;
                                            break;
                                        }
                                    }
                            ...
                            <tr>
                                <td>
                                    <label>
                                        <input type="checkbox" name="id_dotdanhgia[]" value="<?php echo $ds_dgcbcc_dotdanhgia[$i]['id']; ?>" <?php if($check==1) echo 'checked'; ?>>
                                        <span class="lbl"><?php echo $ds_dgcbcc_dotdanhgia[$i]['name']; ?></span>   
                                    </label>                                        
                                </td>
                            </tr>
                            <?php } ?>
...
<div class="modal-footer">
	<span class="btn btn-small btn-success" id="btn_dgcbcc_loaicongviec_luu">Lưu</span>
	<button class="btn btn-small btn-danger" data-dismiss="modal">Quay lại</button>
</div>
<script>
	jQuery(document).ready(function($){
		$('#form_dgcbcc_loaicongviec').validate({
            rules:{
        ...
        $('#btn_dgcbcc_loaicongviec_luu').on('click',function(){
        	if($('#form_dgcbcc_loaicongviec').valid()==true){
        		$.blockUI();
        		$.ajax({
        			type: 'post',
        			url: 'index.php?option=com_danhmuc&controller=dgcbcc_loaicongviec&task=save_dgcbcc_loaicongviec',
        			data: $('#form_dgcbcc_loaicongviec').serialize(),
