...
?>
<div id="ds_mucdophuctap_xeploai">
	<h2 class="header">Mức độ phức tạp
		<div class="pull-right">
			<span class="btn btn-small btn-success" id="btn_themmoi_mucdophuctap" href="#modal-form" data-toggle="modal">Thêm mới</span>
			<span class="btn btn-small btn-danger" id="btn_xoanhieu_mucdophuctap">Xóa</span>
		</div>
	</h2>
	<div class="row-fluid pull-right">
		<div class="span7"></div>
		<div class="span5">
			<div class="span2" style="padding-top:1%">Đợt</div>
			<div class="span5">
			...
			<div class="span2" style="padding-top:1%">Năm</div>
			<div class="span3">
	...
	<div id="table_mucdophuctap_xeploai"></div>
</div>
<?php if($format==''){ ?>
<div id="content"></div>
...
<script>
	jQuery(document).ready(function($){
		$('#select_id_dotdanhgia').on('change',function(){
			...
			$('#table_mucdophuctap_xeploai').load('index.php?option=com_danhmuc&controller=danhgia&task=table_mucdophuctap_xeploai&format=raw&id_dotdanhgia='+id_dotdanhgia,function(){
				$.unblockUI();
			});
		});
		...
		$('#btn_themmoi_mucdophuctap').on('click',function(){
			$('#modal-form').html('');
	    	$('#modal-form').load('<?php echo JURI::base(true);?>/index.php?option=com_danhmuc&controller=danhgia&task=themmoi_mucdophuctap&format=raw', function(){
	    	});
		});
		$('#btn_xoanhieu_mucdophuctap').on('click',function(){
			var array_id = [];
			$('.array_id_mucdophuctap:checked').each(function(){
			...
			if(array_id.length>0){
				if(confirm("Bạn có muốn xóa không?")){
					...
					$.ajax({
						type: 'post',
						url: 'index.php?option=com_danhmuc&controller=mucdophuctap&task=xoanhieu_mucdophuctap',
				...
				loadNoticeBoardError('Thông báo','Vui lòng chọn ít nhất một mức độ để xóa');
				return false;
			}
		});
	});
