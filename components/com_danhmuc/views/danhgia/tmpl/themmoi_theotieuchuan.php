<?php 
	...
	if($id>0){
		$theotieuchuan = $this->theotieuchuan;
...
<div class="modal-body overflow-visible" style="overflow:scroll">
    <div id="modal-content" class="slim-scroll" data-height="350">
		<form class="form-horizontal" id="form_theotieuchuan" enctype="multipart/form-data">
				...
				<div class="tab-content">
					<div id="content_thongtintieuchuan" class="tab-pane in active">
						<div class="row-fluid control-group">
							...
							<div class="controls span9">
								<input type="text" class="span12" name="frm[name]" autocomplete="off" value="<?php if($id>0) echo $theotieuchuan['name']; ?>">
								<input type="hidden" name="frm[id]" value="<?php if($id>0) echo $theotieuchuan['id']; ?>">
							</div>
						</div>
						<div class="row-fluid control-group">
							<div class="span3 control-label">Mã tiêu chuẩn (<span style="color:red">*</span>)</div>
							<div class="controls span9">
								<input type="text" class="span12" name="frm[code]" autocomplete="off" value="<?php if($id>0) echo $theotieuchuan['code']; ?>">
							</div>
						</div>
						<div class="row-fluid control-group">
							<div class="span3 control-label">Hệ số (<span style="color:red">*</span>)</div>
							<div class="controls span9">
								<input type="text" class="span12" name="frm[heso]" autocomplete="off" value="<?php if($id>0) echo $theotieuchuan['heso']; ?>">
							</div>
						</div>
						<div class="row-fluid control-group">
							...
							<div class="controls span9">
								<select class="span12" name="frm[is_canhan]">
									...
									<option value="0" <?php if($id>0&&$theotieuchuan['is_canhan']==0) echo 'selected'; ?>>Không</option>
									<option value="1" <?php if($id>0&&$theotieuchuan['is_canhan']==1) echo 'selected'; ?>>Có</option>
								</select>
							</div>
						</div>
						<div class="row-fluid control-group">
							...
							<div class="controls span9">
								<select class="span12" name="frm[published]">
									<option value="">--Chọn--</option>
									<option value="0" <?php if($id>0&&$theotieuchuan['published']==0) echo 'selected'; ?>>Không</option>
									<option value="1" <?php if($id>0&&$theotieuchuan['published']==1) echo 'selected'; ?>>Có</option>
								</select>
							</div>
						</div>
					</div>
					...
					<div id="content_cauhinhloaicongviec" class="tab-pane">
						<?php for($i=0;$i<count($ds_loaicongviec);$i++){ ?>
							<?php 
								$check = 0;
								$arr_loaicongviec = explode(',',$theotieuchuan['ids_loaicongviec']);
								if($id>0&&count($arr_loaicongviec)>0){				
									for($j=0;$j<count($arr_loaicongviec);$j++){
										if($arr_loaicongviec[$j]==$ds_loaicongviec[$i]['id']){
											$check = 1;
										}
									}
								}
							...
							<div class="row-fluid control-group">
								<label>
									<input type="checkbox" value="<?php echo $ds_loaicongviec[$i]['id']; ?>" name="ids_loaicongviec[]" <?php if($id>0&&$check==1) echo 'checked'; ?>>
									<span class="lbl"><?php echo $ds_loaicongviec[$i]['name']; ?></span>
								</label>
							</div>
						<?php }?>
					</div>
...
<script>
	jQuery(document).ready(function($){
        ...
        $('#btn_dgcbcc_luu').on('click',function(){
        	if($('#form_theotieuchuan').valid()==true){
        		$.blockUI();
        		$.ajax({
        			type: 'post',
        			url: 'index.php?option=com_danhmuc&controller=theotieuchuan&task=luu_theotieuchuan',
        			data: $('#form_theotieuchuan').serialize(),
