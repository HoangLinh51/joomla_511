<?php 
	...
	if($id>0){
		$tieuchi_phanloai = $this->tieuchi_phanloai;
...
<div class="modal-body" style="height:300px">
	...
	<form class="form-horizontal" id="form_tieuchi_liet" enctype="multipart/form-data">
					...
					<div class="row-fluid">
						<div class="control-group">
							...
							<div class="controls span9">
								<input type="text" id="tieuchi_name" name="frm[name]" class="span12" value="<?php if($id>0) echo $tieuchi_phanloai['name']; ?>">
								<input type="hidden" id="tieuchi_id" name="frm[id]" value="<?php if($id>0) echo $tieuchi_phanloai['id']; ?>">
							</div>
						</div>
						<div class="control-group">
							...
							<div class="controls span9">
								<label>
									<input type="checkbox" value="1" name="frm[published]" <?php if($id>0&&$tieuchi_phanloai['published']==1) echo 'checked'; ?>>
							...
							<tr>
								...
								<td>
									<select class="chzn-select id_dotdanhgia" name="id_dotdanhgia[]" data-stt="<?php echo $stt; ?>">
										<option value="">--Chọn--</option>
										<?php for($j=0;$j<count($ds_dotdanhgia);$j++){ ?>
										<option value="<?php echo $ds_dotdanhgia[$j]['id']; ?>" <?php if($ds_dgcbcc_fk_theonhiemvu_tieuchi_phanloai[$i]['id_dotdanhgia']==$ds_dotdanhgia[$j]['id']) echo 'selected'; ?>>
											<?php echo $ds_dotdanhgia[$j]['name']; ?>		
										</option>
										<?php } ?>
									</select>
								</td>
...
<div class="modal-footer">
	<span class="btn btn-small btn-success" id="btn_tieuchi_liet_luu">Lưu</span>
	<button class="btn btn-small btn-danger" data-dismiss="modal">Quay lại</button>
</div>
<script>
	jQuery(document).ready(function($){
		$('.chzn-select').chosen({"width":"100%","search_contains":true});
		$('#form_tieuchi_liet').validate({
            rules:{
		...
		$('#btn_add_row_nhiemvu_dotdanhgia').on('click',function(){
			...
			var select_dotdanhgia = '<select class="chzn-select id_dotdanhgia1" name="id_dotdanhgia[]" data-stt="'+stt+'">';
			select_dotdanhgia += '<option value="">--Chọn--</option>';
			for(var i=0;i<ds_dotdanhgia.length;i++){
				select_dotdanhgia += '<option value="'+ds_dotdanhgia[i]['id']+'">'+ds_dotdanhgia[i]['name']+'</option>';
			}
		...
		$('body').delegate('.id_dotdanhgia1','change',function(){
			...
			if(id_dotdanhgia>0){
				$.ajax({
					type: 'post',
					url: 'index.php?option=com_danhmuc&controller=tieuchiliet&task=find_xeploai_by_id_dotdanhgia',
		...
		$('.id_dotdanhgia').on('change',function(){
			var id_dotdanhgia = $(this).val();
			var stt = $(this).data('stt');
			var id = $('#tieuchi_id').val();
			// console.log(stt);
		...
		$('#btn_tieuchi_liet_luu').on('click',function(){
			if($('#form_tieuchi_liet').valid()==true){
				$.blockUI();
				...
				if(tieuchi_name==''){
					...
					$.ajax({
						type: 'post',
						url: 'index.php?option=com_danhmuc&controller=tieuchiliet&task=luu_tieuchi_liet',
						data: $('#form_tieuchi_liet').serialize(),
						success:function(data){
							if(data>0){
								$('#modal-form').modal('hide');
								$('#btn_tk_tieuchi_phanloai').click();
