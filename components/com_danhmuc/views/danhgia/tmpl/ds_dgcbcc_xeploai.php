...
?>
<div id="ds_dgcbcc_xeploai">
	<h2 class="header">Xếp loại
		<div class="pull-right">
			<span class="btn btn-small btn-success" id="btn_themmoi_dgcbcc_xeploai" href="#modal-form" data-toggle="modal">Thêm mới</span>
			<span class="btn btn-small btn-danger" id="btn_xoanhieu_dgcbcc_xeploai">Xóa</span>
		</div>
	</h2>
	...
	<div id="table_dgcbcc_xeploai"></div>
</div>
<?php if($format==''){ ?>
<div id="content"></div>
...
<script>
	jQuery(document).ready(function($){
		$('#select_id_dotdanhgia').on('change',function(){
			...
			$('#table_dgcbcc_xeploai').load('index.php?option=com_danhmuc&controller=danhgia&task=table_dgcbcc_xeploai&format=raw&id_dotdanhgia='+id_dotdanhgia,function(){
				$.unblockUI();
			});
		});
		...
		$('#btn_themmoi_dgcbcc_xeploai').on('click',function(){
			$('#modal-form').html('');
	    	$('#modal-form').load('<?php echo JURI::base(true);?>/index.php?option=com_danhmuc&controller=danhgia&task=themmoi_xeploai&format=raw', function(){
	    	});
		});
		$('#btn_xoanhieu_dgcbcc_xeploai').on('click',function(){
			var array_id = [];
			$('.array_id_dgcbcc_xeploai:checked').each(function(){
			...
			if(array_id.length>0){
				if(confirm("Bạn có muốn xóa không?")){
					...
					$.ajax({
						type: 'post',
						url: 'index.php?option=com_danhmuc&controller=dgcbcc_xeploai&task=xoanhieu_dgcbcc_xeploai',
				...
				loadNoticeBoardError('Thông báo','Vui lòng chọn ít nhất một xếp loại để xóa');
				return false;
			}
		});
	});
