<?php 
	...
	if($id_tieuchi>0){
		$dgcbcc_tieuchi_donvi = $this->dgcbcc_tieuchi_donvi;
	}
?>
...
<div class="modal-body overflow-visible">
	...
	<form class="form-horizontal" id="form_change_dgcbcc_tieuchi_donvi" enctype="multipart/form-data">
		<?php echo JHtml::_( 'form.token' ); ?>
		<div class="row-fluid">
			<div class="control-group">
				<label class="control-label span3">Đợt đánh giá bắt đầu (<span style="color:red">*</span>)</label>
				<div class="controls span9">
					<select class="span12 chzn-select" name="frm[id_dotdanhgia_batdau]">
						<option value="">--Chọn--</option>
						<?php for($i=0;$i<count($ds_dotdanhgia_thang);$i++){ ?>
						<option value="<?php echo $ds_dotdanhgia_thang[$i]['id_dotdanhgia']; ?>" <?php if($id_tieuchi>0&&$dgcbcc_tieuchi_donvi['id_dotdanhgia_batdau']==$ds_dotdanhgia_thang[$i]['id_dotdanhgia']) echo 'selected'; ?>>
			...
			<div class="control-group">
				...
				<div class="controls span9">
					<select class="span12 chzn-select" name="frm[id_dotdanhgia_ketthuc]">
						<option value="">--Chọn--</option>
						<?php for($i=0;$i<count($ds_dotdanhgia_thang);$i++){ ?>
						<option value="<?php echo $ds_dotdanhgia_thang[$i]['id_dotdanhgia']; ?>" <?php if($id_tieuchi>0&&$dgcbcc_tieuchi_donvi['id_dotdanhgia_ketthuc']==$ds_dotdanhgia_thang[$i]['id_dotdanhgia']) echo 'selected'; ?>>
							<?php echo $ds_dotdanhgia_thang[$i]['name']; ?>
						</option>
						<?php }?>
					</select>
			...
			<div class="control-group">
				...
				<div class="controls span9">
					<input type="text" class="span12" autocomplete="off" name="frm[name]" value="<?php if($id_tieuchi>0) echo $dgcbcc_tieuchi_donvi['name']; ?>">
				</div>
			</div>
		</div>
	</form>
...
<script>
	jQuery(document).ready(function($){
		...
		$('#form_change_dgcbcc_tieuchi_donvi').validate({
            rules:{
        ...
        $('#btn_change_dgcbcc_tieuchi_luu').on('click',function(){
        	if($('#form_change_dgcbcc_tieuchi_donvi').valid()==true){
        		$.blockUI();
        		$.ajax({
        			type: 'post',
        			url: 'index.php?option=com_danhmuc&controller=dgcbcctieuchidonvi&task=luu_dgcbcc_tieuchi_donvi',
        			data: $('#form_change_dgcbcc_tieuchi_donvi').serialize(),
