...
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			...
			<h3 class="smaller lighter blue no-margin"><?php echo ($info['id'])>0?"Hiệu chỉnh":"Thêm mới"?> Mốc thời gian</h3>
		</div>
		<div class="modal-body">
			<form class="form-horizontal" method="POST" name="frm_baucumocthoigian" id="frm_baucumocthoigian">
		...
		<div class="modal-footer">
			...
			<span class="btn btn-sm btn-primary pull-right" id="btn_submit_baucumocthoigian">
				<i class="ace-icon fa fa-times"></i>
				Lưu
			</span>
		</div>
...
jQuery(document).ready(function($){
	...
	$('#btn_submit_baucumocthoigian').click(function(){
		$('.gritter-item-wrapper').remove();
		var flag = $('#frm_baucumocthoigian').valid();
		if(flag == true){
			$.blockUI();
			formData = $('#frm_baucumocthoigian').serialize();
			$.ajax({
				type: 'post',
				data: formData,
				url: 'index.php?option=com_danhmuc&controller=baucumocthoigian&task=save&format=raw&',
