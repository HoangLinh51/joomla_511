...
$dantoc = Core::loadAssocList('nat_code','*','id!='.(int)$info['id'],'name asc');
?>
<style>
...
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			...
			<h3 class="smaller lighter blue no-margin"><?php echo mb_strlen($info['id'])>0?"Hiệu chỉnh":"Thêm mới"?> Dân tộc</h3>
		</div>
		<div class="modal-body">
			<form class="form-horizontal" method="POST" name="frm_dantoc" id="frm_dantoc">
				...
				<div class="row-fluid">
					<div class="control-group">
						<label class="control-label" for="type">Là nhánh con của <span style="color:red;">*</span></label>
						<div class="controls">
							<select name="type" id="type">
								<option value="">-- Chọn nhánh cha --</option>
								<?php for($i=0; $n=count($dantoc), $i<$n; $i++){?>
									<option value="<?php echo $dantoc[$i]['id']?>" <?php echo $info['parent']==$dantoc[$i]['id']?'selected':'';?>><?php echo $dantoc[$i]['name']?></option>
								<?php }?>
							</select>
						</div>
					</div>
		...
		<div class="modal-footer">
			...
			<span class="btn btn-sm btn-primary pull-right" id="btn_submit_dantoc">
				<i class="ace-icon fa fa-times"></i>
				Lưu
			</span>
		</div>
...
jQuery(document).ready(function($){
	...
	$('#btn_submit_dantoc').click(function(){
		$('.gritter-item-wrapper').remove();
		var flag = $('#frm_dantoc').valid();
		if(flag == true){
			$.blockUI();
			formData = $('#frm_dantoc').serialize();
			$.ajax({
				type: 'post',
				data: formData,
				url: 'index.php?option=com_danhmuc&controller=dantoc&task=save&format=raw&',
