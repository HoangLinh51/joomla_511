...
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			...
			<h3 class="smaller lighter blue no-margin"><?php echo $info['id']>0?"Hiệu chỉnh":"Thêm mới"?> Mức độ tham gia</h3>
		</div>
		<div class="modal-body">
			<form class="form-horizontal" method="POST" name="frm_vtvlmucdothamgia" id="frm_vtvlmucdothamgia">
				<div class="row-fluid">
		...
		<div class="modal-footer">
			...
			<span class="btn btn-sm btn-primary pull-right" id="btn_submit_vtvlmucdothamgia">
				<i class="ace-icon fa fa-times"></i>
				Lưu
			</span>
		</div>
...
jQuery(document).ready(function($){
	...
	$('#btn_submit_vtvlmucdothamgia').click(function(){
		$('.gritter-item-wrapper').remove();
		var flag = $('#frm_vtvlmucdothamgia').valid();
		if(flag == true){
			$.blockUI();
			formData = $('#frm_vtvlmucdothamgia').serialize();
			$.ajax({
				type: 'post',
				data: formData,
				url: 'index.php?option=com_danhmuc&controller=vtvlmucdothamgia&task=save&format=raw&',
