...
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			...
			<h3 class="smaller lighter blue no-margin"><?php echo $info['id']>0?"Hiệu chỉnh":"Thêm mới"?> Điều kiện làm việc</h3>
		</div>
		<div class="modal-body">
			<form class="form-horizontal" method="POST" name="frm_vtvldieukienlamviec" id="frm_vtvldieukienlamviec">
				<div class="row-fluid">
					...
					<div class="control-group">
						...
						<div class="controls">
							<select name="type" id="type">
								<option value="1" <?php echo $info['type']==1?'selected':"";?>>Nhập nội dung</option>
								<option value="2" <?php echo $info['type']==2?'selected':"";?>>Chọn nội dung</option>
							</select>
						</div>
					</div>
				</div>		
				...
				<div class="row-fluid">
					<div class="control-group">
						<label class="control-label" for="description">Mô tả <span style="color:red;">*</span></label>
						<div class="controls">
							<div class="input-append">
								<textarea style="width: 100%" name="description" id="description"><?php echo $info['description']?></textarea>
							</span>
						</div>
						</div>
					</div>
		...
		<div class="modal-footer">
			...
			<span class="btn btn-sm btn-primary pull-right" id="btn_submit_vtvldieukienlamviec">
				<i class="ace-icon fa fa-times"></i>
				Lưu
			</span>
		</div>
...
jQuery(document).ready(function($){
	...
	$('#frm_vtvldieukienlamviec').validate({
		...
		messages:{
			...
			"description": {	    
			  	required: 'Vui lòng nhập <b>Mô tả</b>',
	      	},
		}
	});	
	$('#btn_submit_vtvldieukienlamviec').click(function(){
		$('.gritter-item-wrapper').remove();
		var flag = $('#frm_vtvldieukienlamviec').valid();
		if(flag == true){
			$.blockUI();
			formData = $('#frm_vtvldieukienlamviec').serialize();
			$.ajax({
				type: 'post',
				data: formData,
				url: 'index.php?option=com_danhmuc&controller=vtvldieukienlamviec&task=save&format=raw&',
