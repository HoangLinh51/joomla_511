...
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			...
			<h3 class="smaller lighter blue no-margin"><?php echo mb_strlen($info['id'])>0?"Hiệu chỉnh":"Thêm mới"?> Nơi cấp</h3>
		</div>
		<div class="modal-body">
			<form class="form-horizontal" method="POST" name="frm_noicap" id="frm_noicap">
				...
				<div class="row-fluid">
					<div class="control-group">
						<label class="control-label" for="ma">Mã <span style="color:red;">*</span></label>
						<div class="controls">
							<input type="text" name="ma" id="ma" style="width:80%;" value="<?php echo $info['ma']?>"/>
						</div>
					</div>
				</div>
				<div class="row-fluid">
					<div class="control-group">
						<label class="control-label" for="cap">Cấp <span style="color:red;">*</span></label>
						<div class="controls">
							<input type="text" name="cap" id="cap" style="width:80%;" value="<?php echo $info['cap']?>"/>
						</div>
					</div>
				</div>
				<div class="row-fluid">
					<div class="control-group">
						...
						<div class="controls">
							<select name="type" id="type">
								<option value="">-- Chọn loại nơi cấp--</option>
								<option value="0" <?php echo $info['type']==0?'selected':'';?>>Dùng chung</option>
								<option value="1" <?php echo $info['type']==1?'selected':'';?>>Nơi cấp CMND</option>
								<option value="2" <?php echo $info['type']==2?'selected':'';?>>Nơi cấp thẻ Căn cước</option>
							</select>
						</div>
					</div>
				</div>
		...
		<div class="modal-footer">
			...
			<span class="btn btn-sm btn-primary pull-right" id="btn_submit_noicap">
				<i class="ace-icon fa fa-times"></i>
				Lưu
			</span>
		</div>
...
jQuery(document).ready(function($){
	...
	$('#btn_submit_noicap').click(function(){
		$('.gritter-item-wrapper').remove();
		var flag = $('#frm_noicap').valid();
		if(flag == true){
			$.blockUI();
			formData = $('#frm_noicap').serialize();
			$.ajax({
				type: 'post',
				data: formData,
				url: 'index.php?option=com_danhmuc&controller=noicap&task=save&format=raw&',
