...
$tinhthanh = Core::loadAssocList('city_code','*','code>0 and daxoa=0','(code>0) asc, muctuongduong is null asc, name is null asc');
?>
<style>
...
<div class="modal-dialog">
	<div class="modal-content">
		...
		<div class="modal-body" style="height: 500px;">
			<form class="form-horizontal" method="POST" name="frm_quanhuyen" id="frm_quanhuyen">
				...
				<div class="row-fluid">
					<div class="control-group">
						<label class="control-label" for="cadc_code">Thuộc tỉnh/thành <span style="color:red;">*</span></label>
						<div class="controls">
							<select name="cadc_code" id="cadc_code" class="chosen">
								<option value="">-- Chọn tỉnh/thành --</option>
								<?php for($i=0; $n=count($tinhthanh), $i<$n; $i++){?>
								<option value="<?php echo $tinhthanh[$i]['code']?>" <?php echo $tinhthanh[$i]['code']==$info['cadc_code']?'selected':'';?>><?php echo $tinhthanh[$i]['name']?></option>
								<?php }?>
							</select>
						</div>
					</div>
					...
					<div class="control-group">
						...
						<div class="controls">
							<select name="muctuongduong" id="muctuongduong">
								...
								<option value="1" <?php echo $info['muctuongduong']==1?'selected':'';?>>Thành phố</option>
								<option value="2" <?php echo $info['muctuongduong']==2?'selected':'';?>>Thị xã</option>
								<option value="3" <?php echo $info['muctuongduong']==3?'selected':'';?>>Quận</option>
								<option value="4" <?php echo $info['muctuongduong']==4?'selected':'';?>>Huyện</option>
							</select>
						</div>
					</div>
				</div>
		...
		<div class="modal-footer">
			...
			<span class="btn btn-sm btn-primary pull-right" id="btn_submit_quanhuyen">
				<i class="ace-icon fa fa-times"></i>
				Lưu
			</span>
		</div>
...
jQuery(document).ready(function($){
	...
	$('#btn_submit_quanhuyen').click(function(){
		$('.gritter-item-wrapper').remove();
		var flag = $('#frm_quanhuyen').valid();
		if(flag == true){
			$.blockUI();
			formData = $('#frm_quanhuyen').serialize();
			$.ajax({
				type: 'post',
				data: formData,
				url: 'index.php?option=com_danhmuc&controller=quanhuyen&task=save&format=raw&',
