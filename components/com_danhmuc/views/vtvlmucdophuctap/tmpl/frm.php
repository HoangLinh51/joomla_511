...
<div class="modal-dialog">
	<div class="modal-content">
		<div class="modal-header">
			...
			<h3 class="smaller lighter blue no-margin"><?php echo $info['id']>0?"Hiệu chỉnh":"Thêm mới"?> Mức độ phức tạp</h3>
		</div>
		<div class="modal-body">
			<form class="form-horizontal" method="POST" name="frm_vtvlmucdophuctap" id="frm_vtvlmucdophuctap">
				<div class="row-fluid">
		...
		<div class="modal-footer">
			...
			<span class="btn btn-sm btn-primary pull-right" id="btn_submit_vtvlmucdophuctap">
				<i class="ace-icon fa fa-times"></i>
				Lưu
			</span>
		</div>
...
jQuery(document).ready(function($){
	...
	$('#btn_submit_vtvlmucdophuctap').click(function(){
		$('.gritter-item-wrapper').remove();
		var flag = $('#frm_vtvlmucdophuctap').valid();
		if(flag == true){
			$.blockUI();
			formData = $('#frm_vtvlmucdophuctap').serialize();
			$.ajax({
				type: 'post',
				data: formData,
				url: 'index.php?option=com_danhmuc&controller=vtvlmucdophuctap&task=save&format=raw&',
