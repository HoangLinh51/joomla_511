<?php 
	class DanhmucControllerDgcbcc_tieuchiliet extends DanhmucController{
		...
		public function luu_dgcbcc_tieuchi_liet(){
			...
			$model = Core::model('Danhmuchethong/DgcbccTieuchiliet');
			if($form['id']==''){
				$id_tieuchi_phanloai = $model->add_dgcbcc_tieuchi_phanloai($form);
				...
				$id_tieuchi_phanloai = $model->update_dgcbcc_tieuchi_phanloai($form);
				if($id_tieuchi_phanloai>0){
		...
		public function delete_dgcbcc_tieuchi_liet(){
			$jinput = JFactory::getApplication()->input;
			$id = $jinput->getInt('id',0);
			if($id>0){
				$model = Core::model('Danhmuchethong/DgcbccTieuchiliet');
				$model->delete_dgcbcc_fk_theonhiemvu_tieuchi_phanloai($id);
				Core::printJson($model->delete_dgcbcc_tieuchi_liet($id));exit;
			}
			Core::printJson(false);exit;
		}
		public function xoanhieu_dgcbcc_tieuchi_liet(){
			$jinput = JFactory::getApplication()->input;
			$id = $jinput->getInt('id',0);
			if(count($id)>0){
				$model = Core::model('Danhmuchethong/DgcbccTieuchiliet');
				for($i=0;$i<count($id);$i++){
					...
					$result[] = $model->delete_dgcbcc_tieuchi_liet($id[$i]);
				}
