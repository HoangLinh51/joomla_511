<?php 
	class DanhmucControllerDgcbcc_theotieuchuan extends DanhmucController{
		public function luu_dgcbcc_theotieuchuan(){
			...
			$model = Core::model('Danhmuchethong/Dgcbcc_theotieuchuan');
			if($form['id']==''){
				$id_tieuchuan = $model->add_dgcbcc_theotieuchuan($form,$ids_loaicongviec);
				...
				$id_tieuchuan = $model->update_dgcbcc_theotieuchuan($form,$ids_loaicongviec);
		...
		public function delete_dgcbcc_theotieuchuan(){
			$jinput = JFactory::getApplication()->input;
			$id = $jinput->getInt('id',0);
			if($id>0){
				$model = Core::model('Danhmuchethong/Dgcbcc_theotieuchuan');
				Core::printJson($model->delete_dgcbcc_theotieuchuan($id));exit;
			}
			Core::printJson(false);exit;
		}
		public function xoanhieu_dgcbcc_theotieuchuan(){
			$jinput = JFactory::getApplication()->input;
			$id = $jinput->getInt('id',0);
			$model = Core::model('Danhmuchethong/Dgcbcc_theotieuchuan');
			if(count($id)>0){
				for($i=0;$i<count($id);$i++){
					$result[] = $model->delete_dgcbcc_theotieuchuan($id[$i]);
				}
