      ...
      <table id="table-baoluc" class="table table-striped table-bordered" style="table-layout: fixed; width: 100%; margin: 0px">
        <thead class="table-primary">
          <tr>
            ...
            <th style="width: 150px; text-align: center;" rowspan="2">Thông tin vụ việc</th>
...
<script>
  jQuery(document).ready(function($) {
            ...
            response.data.forEach((item, index) => {
              ...
              const html = `
                        <tr>
                            ...
                            <td class="align-middle">${item.coquanxuly || ''}<br>${item.mavuviec || ''}<br>${item.ngayxuly2 || ''}<br>${tinhtrangText}</td>
    ...
    $('.dsBaoluc').on('click', '.btn_edit_baoluc', function() {
      ...
      $('#modal_nguoigay_id').val(data.nguoigay_id);
      $('#modal_nannhan_id').val(data.nannhan_id);
      ...
      $('#modal_nguoigay_id').trigger('change');
      $('#modal_nannhan_id').trigger('change');
      $('#modal_bienphap').trigger('change');
      $('#modal_hotro').trigger('change');
      $('#modal_tinhtrang').trigger('change');
