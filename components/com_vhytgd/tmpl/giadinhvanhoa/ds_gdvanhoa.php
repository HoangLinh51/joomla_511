            ...
            <?php if (!empty($this->items)): ?>
                <?php foreach ($this->items as $i => $item): ?>
                    <tr>
                        ...
                        <td style="vertical-align:middle;"><?php echo htmlspecialchars($item['nam']); ?></td>

                        <td style="vertical-align:middle;text-align: center;">
                            <span><?php echo htmlspecialchars($item['so_nguoi']); ?></span>
                            <i class="fas fa-eye btn_eye" style="cursor: pointer; margin-left: 10px; color: #007bff;" data-thonto-id="<?php echo $item['thonto_id']; ?>" data-nam="<?php echo $item['nam']; ?>" title="Xem chi tiết"></i>
                        </td>
                        <td style="vertical-align:middle;text-align: center;">
                            <div class="btn-group" role="group">
                                <span class="btn btn-sm btn_hieuchinh" data-id="<?php echo $item['giadinhvanhoa_id']; ?>" data-title="Hiệu chỉnh" style="cursor: pointer;">
                                    <i class="fas fa-pencil-alt"></i>
                                </span>
                                <span style="padding: 0 5px;font-size:20px;color:#ccc">|</span>
                                <span class="btn btn-sm btn_xoa" data-id="<?php echo $item['giadinhvanhoa_id']; ?>" data-title="Xóa" style="cursor: pointer;">
...
<script type="text/javascript">
    jQuery(document).ready(function($) {
        ...
        function loadDetail(thontoID, nam) {
            $("#overlay").fadeIn(300);
            var params = {
                option: 'com_vhytgd',
                view: 'giadinhvanhoa',
        ...
        $('body').on('click', '.btn_xoa', function() {
            ...
            bootbox.confirm({
                ...
                callback: function(result) {
                    ...
                    // Gửi yêu cầu AJAX
                    $.ajax({
                        url: Joomla.getOptions('system.paths').base + '/index.php?option=com_vhytgd&controller=giadinhvanhoa&task=removeGiaDinhVanHoa',
                        type: 'POST',
                        data: {
        ...
        $('body').on('click', '.btn_eye', function(e) {
            e.preventDefault();
            var thontoId = $(this).data('thonto-id');
            var nam = $(this).data('nam');
            loadDetail(thontoId, nam);
        });
    });
</script>
