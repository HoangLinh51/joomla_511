...
<?php if ($this->isCapnhat == 1): ?>
  <form id="imageUploadForm" name="frmUploadOne" enctype="multipart/form-data" action="<?php echo Uri::root(true); ?>/index.php?option=com_core&controller=attachment&task=uploadSingleImage" method="post" target="tftemp<?php echo $this->idObject ?>">
    <div class="mb-2">
      <label for="uploadfileone" class="btn btn-sm btn-primary">
        <i class="fas fa-upload"></i> Chọn ảnh
      </label>
      <input type="file" id="uploadfileone" name="uploadfile" accept=".jpg, .jpeg, .png"
        onchange="document.frmUploadOne.submit();" hidden />
    </div>
