...
$modelQuanTriHeThong = Core::model('QuanTriHeThong/QuanTriHeThong');
$item = $this->item;
?>
	...
	<div class="card">
		<div class="card-header">
			...
			<div>
				...
				<?php endif; ?>
				<a href="<?= Route::_('index.php?option=com_quantrihethong&view=quantrihethong&task=ds_quantrihethong') ?>" class="btn btn-secondary">Quay lại</a>
			</div>
		</div>
		<div class="card-body">
			<!-- Thông tin lỗi -->
...
<script>
	document.addEventListener('DOMContentLoaded', function() {
		...
		$('body').on('click', '.btn-submit', function() {
			...
			const url = "<?= Route::_('index.php?option=com_quantrihethong&task=quantrihethong.confirm_reason') ?>";
			...
			fetch(url, {
				...
				.then(data => {
					...
					if (isSuccess) {
						setTimeout(() => {
							window.location.href = '/index.php/component/quantrihethong/?view=quantrihethong&task=ds_quantrihethong';
						}, 500);
					}
				})
				.catch(error => {
