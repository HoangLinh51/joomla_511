 ...
 * @since  1.6
 */
class HangthuongbinhController extends AdminController
{
    /**
     * Constructor.
     *
     ...
     * @since   1.6
     */
    public function getModel($name = 'Hangthuongbinh', $prefix = 'Administrator', $config = ['ignore_request' => true])
    {
    ...
    public function delete()
    {
        ...
        if (empty($ids)) {
            ...
            // Get the model.
            // Cách 2
            $model_core = Core::model('Danhmuchethong/Thuongbinh');
            if ($model_core->xoanhieuparty_pos($ids)) {
                $this->setMessage(Text::plural('Xóa thành công', ""));
            }
        }

        $this->setRedirect('index.php?option=com_danhmuc&view=hangthuongbinh');
    }
    ...
    public function save2new($key = null, $urlVar = null)
    {
        ...
        /** @var \Joomla\Component\Danhmuc\Administrator\Model\HangthuongbinhModel $model */
        $model = $this->getModel('Hangthuongbinh', 'Administrator');
        ...
        if ($return === false) {
            $this->setRedirect(Route::_('index.php?option=com_danhmuc&view=hangthuongbinh', false));
            return false;
        }
        ...
        switch ($this->input->getCmd('task')) {
            case 'save2new':
            default:
                $this->setRedirect(Route::_('index.php?option=com_danhmuc&view=hangthuongbinh&layout=edit', false));
                break;
        }
    }
    ...
    public function publish()
    {
        ...
        $this->app->enqueueMessage(Text::_('Lưu thành công'), 'message');
        $this->setRedirect(Route::_('index.php?option=com_danhmuc&view=hangthuongbinh', true));
    }   
}