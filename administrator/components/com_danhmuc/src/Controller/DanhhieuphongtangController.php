 ...
 * @since  1.6
 */
class DanhhieuphongtangController extends AdminController
{
    /**
     * Constructor.
     *
     ...
     * @since   1.6
     */
    public function getModel($name = 'Danhhieuphongtang', $prefix = 'Administrator', $config = ['ignore_request' => true])
    {
    ...
    public function delete()
    {
        ...
        if (empty($ids)) {
            ...
            // Get the model.
            // Cách 2
            $model_core = Core::model('Danhmuchethong/Danhhieuphongtang');
            if ($model_core->xoanhieuparty_pos($ids)) {
                $this->setMessage(Text::plural('Xóa thành công', ""));
            }
        }

        $this->setRedirect('index.php?option=com_danhmuc&view=danhhieuphongtang');
    }
    ...
    public function save2new($key = null, $urlVar = null)
    {
        ...
        /** @var \Joomla\Component\Danhmuc\Administrator\Model\DanhhieuphongtangModel $model */
        $model = $this->getModel('Danhhieuphongtang', 'Administrator');
        ...
        if ($return === false) {
            $this->setRedirect(Route::_('index.php?option=com_danhmuc&view=danhhieuphongtang', false));
            return false;
        }
        ...
        switch ($this->input->getCmd('task')) {
            case 'save2new':
            default:
                $this->setRedirect(Route::_('index.php?option=com_danhmuc&view=danhhieuphongtang&layout=edit', false));
                break;
        }
    }
    ...
    public function save($key = null, $urlVar = null)
    {
        ...
        /** @var \Joomla\Component\Danhmuc\Administrator\Model\TpxuathanModel $model */
        $model = $this->getModel('Danhhieuphongtang', 'Administrator');
        $data  = $this->input->post->get('jform', [], 'array');
        $code = $this->input->getInt('code');
        // var_dump($data);exit;
    ...
    public function publish()
    {
        ...
        $this->app->enqueueMessage(Text::_('Lưu thành công'), 'message');
        $this->setRedirect(Route::_('index.php?option=com_danhmuc&view=danhhieuphongtang', true));
    }   
}