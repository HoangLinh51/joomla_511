 ...
 * @since  1.6
 */
class GioitinhController extends AdminController
{
    /**
     * Constructor.
     *
     ...
     * @since   1.6
     */
    public function getModel($name = 'Gioitinh', $prefix = 'Administrator', $config = ['ignore_request' => true])
    {
    ...
    public function delete()
    {
        ...
        if (empty($ids)) {
            ...
            // Get the model.
            // Cách 2
            $model_core = Core::model('Danhmuchethong/Gioitinh');
            if ($model_core->xoanhieuparty_pos($ids)) {
                $this->setMessage(Text::plural('Xóa thành công', ""));
            }
        }

        $this->setRedirect('index.php?option=com_danhmuc&view=gioitinh');
    }
    ...
    public function save2new($key = null, $urlVar = null)
    {
        ...
        /** @var \Joomla\Component\Danhmuc\Administrator\Model\GioitinhModel $model */
        $model = $this->getModel('Gioitinh', 'Administrator');
        ...
        if ($return === false) {
            $this->setRedirect(Route::_('index.php?option=com_danhmuc&view=gioitinh', false));
            return false;
        }
        ...
        switch ($this->input->getCmd('task')) {
            case 'save2new':
            default:
                $this->setRedirect(Route::_('index.php?option=com_danhmuc&view=gioitinh&layout=edit', false));
                break;
        }
    }
    ...
    public function publish()
    {
        ...
        $this->app->enqueueMessage(Text::_('Lưu thành công'), 'message');
        $this->setRedirect(Route::_('index.php?option=com_danhmuc&view=gioitinh', true));
    }   
}