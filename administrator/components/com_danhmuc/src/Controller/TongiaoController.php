 ...
 * @since  1.6
 */
class TongiaoController extends AdminController
{
    /**
     * Constructor.
     *
     ...
     * @since   1.6
     */
    public function getModel($name = 'Tongiao', $prefix = 'Administrator', $config = ['ignore_request' => true])
    {
    ...
    public function delete()
    {
        ...
        if (empty($ids)) {
            ...
            // Get the model.
            // Cách 2
            $model_core = Core::model('Danhmuchethong/Tongiao');
            if ($model_core->xoanhieuparty_pos($ids)) {
                $this->setMessage(Text::plural('Xóa thành công', ""));
            }
        }

        $this->setRedirect('index.php?option=com_danhmuc&view=tongiao');
    }
    ...
    public function save2new($key = null, $urlVar = null)
    {
        ...
        /** @var \Joomla\Component\Danhmuc\Administrator\Model\TongiaoModel $model */
        $model = $this->getModel('Tongiao', 'Administrator');
        ...
        if ($return === false) {
            $this->setRedirect(Route::_('index.php?option=com_danhmuc&view=tongiao', false));
            return false;
        }
        ...
        switch ($this->input->getCmd('task')) {
            case 'save2new':
            default:
                $this->setRedirect(Route::_('index.php?option=com_danhmuc&view=tongiao&layout=edit', false));
                break;
        }
    }
    ...
    public function publish()
    {
        ...
        $this->app->enqueueMessage(Text::_('Lưu thành công'), 'message');
        $this->setRedirect(Route::_('index.php?option=com_danhmuc&view=tongiao', true));
    }   
}