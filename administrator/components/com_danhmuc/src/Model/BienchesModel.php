 ...
 * @since  3.7.0
 */
class BienchesModel extends ListModel
{
     ...
     * @throws  \Exception
     */
    public function getTable($type = 'Bienches', $prefix = 'Joomla\\Component\\Danhmuc\\Administrator\\Table\\', $config = [])
    {
    ...
    protected function getListQuery()
    {
        ...
        $query->from($db->quoteName('bienche') . ' AS a');
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_danhmuc.bienches', 'bienches', ['control' => 'jform', 'load_data' => $loadData]);
    ...
    public function getItem($pk = null)
    {
        ...
        $query = $db->getQuery(true);
        $query->select('code, name, level, status')->from('bienche')->where('code = ' . $pk);
        $db->setQuery($query);
        // echo $query;
    ...
    public function loadFormData()
    {
        ...
        if ($pk > 0) {
            ...
            if (!$data || !$data->code) {
                $filters = (array) $app->getUserState('com_danhmuc.bienches.filter');
                $data = (object) array_merge((array) $filters, (array) $data);
            }
            $this->preprocessData('com_danhmuc.bienches', $data);
        } else {
            $data = (array) $app->getUserState('com_danhmuc.bienches.filter', array());
        }
        return $data;
    }
}
