 ...
 * @since  3.7.0
 */
class GioitinhModel extends ListModel
{
     ...
     * @throws  \Exception
     */
    public function getTable($type = 'Gioitinh', $prefix = 'Joomla\\Component\\Danhmuc\\Administrator\\Table\\', $config = [])
    {
    ...
    protected function getListQuery()
    {
        ...
        $query->from($db->quoteName('danhmuc_gioitinh') . ' AS a');
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_danhmuc.gioitinh', 'gioitinh', ['control' => 'jform', 'load_data' => $loadData]);
    ...
    public function getItem($pk = null)
    {
        ...
        $query = $db->getQuery(true);
        $query->select('code, name, level, status')->from('danhmuc_gioitinh')->where('code = ' . $pk);
        $db->setQuery($query);
    ...
    public function loadFormData()
    {
        ...
        if ($pk > 0) {
            ...
            if (!$data || !$data->code) {
                $filters = (array) $app->getUserState('com_danhmuc.gioitinh.filter');
                $data = (object) array_merge((array) $filters, (array) $data);
            }
            $this->preprocessData('com_danhmuc.gioitinh', $data);
        } else {
            $data = (array) $app->getUserState('com_danhmuc.gioitinh.filter', array());
        }
        return $data;
    }
}
