 ...
 * @since  3.7.0
 */
class NangluongModel extends ListModel
{
     ...
     * @throws  \Exception
     */
    public function getTable($type = 'Nangluong', $prefix = 'Joomla\\Component\\Danhmuc\\Administrator\\Table\\', $config = [])
    {
    ...
    protected function getListQuery()
    {
        ...
        $query->from($db->quoteName('danhmuc_hinhthucnangluong') . ' AS a');
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_danhmuc.nangluong', 'nangluong', ['control' => 'jform', 'load_data' => $loadData]);
    ...
    public function save($data, $pk = null)
    {
        ...
        // $query->insert('danhmuc_hinhthucnangluong')->columns($columns)->values(implode(',',$values));
        // $db->setQuery($query);
        // return $db->execute();
    ...
    public function getItem($pk = null)
    {
        ...
        $query = $db->getQuery(true);
        $query->select('code, name,raise_salary, receive_money, percent, status')->from('danhmuc_hinhthucnangluong')->where('code = ' . $pk);
        $db->setQuery($query);
    ...
    public function loadFormData()
    {
        ...
        if ($pk > 0) {
            ...
            if (!$data || !$data->code) {
                $filters = (array) $app->getUserState('com_danhmuc.nangluong.filter');
                $data = (object) array_merge((array) $filters, (array) $data);
            }
            $this->preprocessData('com_danhmuc.nangluong', $data);
        } else {
            $data = (array) $app->getUserState('com_danhmuc.nangluong.filter', array());
        }
        return $data;
    }
}
