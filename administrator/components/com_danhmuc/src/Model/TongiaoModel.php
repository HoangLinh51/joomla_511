 ...
 * @since  3.7.0
 */
class TongiaoModel extends ListModel
{
     ...
     * @throws  \Exception
     */
    public function getTable($type = 'Tongiao', $prefix = 'Joomla\\Component\\Danhmuc\\Administrator\\Table\\', $config = [])
    {
    ...
    protected function getListQuery()
    {
        ...
        $query->from($db->quoteName('danhmuc_tongiao') . ' AS a');
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_danhmuc.tongiao', 'tongiao', ['control' => 'jform', 'load_data' => $loadData]);
    ...
    public function save($data, $pk = null)
    {
        ...
        // $query->insert('danhmuc_tongiao')->columns($columns)->values(implode(',',$values));
        // $db->setQuery($query);
        // return $db->execute();
    ...
    public function getItem($pk = null)
    {
        ...
        $query = $db->getQuery(true);
        $query->select('code, name, level, status')->from('danhmuc_tongiao')->where('code = ' . $pk);
        $db->setQuery($query);
    ...
    public function loadFormData()
    {
        ...
        if ($pk > 0) {
            ...
            if (!$data || !$data->code) {
                $filters = (array) $app->getUserState('com_danhmuc.tongiao.filter');
                $data = (object) array_merge((array) $filters, (array) $data);
            }
            $this->preprocessData('com_danhmuc.tongiao', $data);
        } else {
            $data = (array) $app->getUserState('com_danhmuc.tongiao.filter', array());
        }
        return $data;
    }
}
