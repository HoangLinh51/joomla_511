 ...
 * @since  3.7.0
 */
class DoituonghuongcsModel extends ListModel
{
     ...
     * @throws  \Exception
     */
    public function getTable($type = 'Doituonghuongcs', $prefix = 'Joomla\\Component\\Danhmuc\\Administrator\\Table\\', $config = [])
    {
    ...
    protected function getListQuery()
    {
        ...
        $query->from($db->quoteName('danhmuc_doituonghuongchinhsach') . ' AS a');
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_danhmuc.doituonghuongcs', 'doituonghuongcs', ['control' => 'jform', 'load_data' => $loadData]);
    ...
    public function save($data, $pk = null)
    {
        ...
        // $query->insert('danhmuc_doituonghuongchinhsach')->columns($columns)->values(implode(',',$values));
        // $db->setQuery($query);
        // return $db->execute();
    ...
    public function getItem($pk = null)
    {
        ...
        $query = $db->getQuery(true);
        $query->select('code, name, level, status')->from('danhmuc_doituonghuongchinhsach')->where('code = ' . $pk);
        $db->setQuery($query);
    ...
    public function loadFormData()
    {
        ...
        if ($pk > 0) {
            ...
            if (!$data || !$data->code) {
                $filters = (array) $app->getUserState('com_danhmuc.doituonghuongcs.filter');
                $data = (object) array_merge((array) $filters, (array) $data);
            }
            $this->preprocessData('com_danhmuc.doituonghuongcs', $data);
        } else {
            $data = (array) $app->getUserState('com_danhmuc.doituonghuongcs.filter', array());
        }
        return $data;
    }
}
