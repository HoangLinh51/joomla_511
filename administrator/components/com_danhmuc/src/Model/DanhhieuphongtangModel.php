 ...
 * @since  3.7.0
 */
class DanhhieuphongtangModel extends ListModel
{
     ...
     * @throws  \Exception
     */
    public function getTable($type = 'Danhhieuphongtang', $prefix = 'Joomla\\Component\\Danhmuc\\Administrator\\Table\\', $config = [])
    {
    ...
    protected function getListQuery()
    {
        ...
        $query->from($db->quoteName('danhmuc_danhhieuphongtang') . ' AS a');
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_danhmuc.danhhieuphongtang', 'danhhieuphongtang', ['control' => 'jform', 'load_data' => $loadData]);
    ...
    public function getItem($pk = null)
    {
        ...
        $query = $db->getQuery(true);
        $query->select('code, name, level, status')->from('danhmuc_danhhieuphongtang')->where('code = ' . $pk);
        $db->setQuery($query);
    ...
    public function loadFormData()
    {
        ...
        if ($pk > 0) {
            ...
            if (!$data || !$data->code) {
                $filters = (array) $app->getUserState('com_danhmuc.danhhieuphongtang.filter');
                $data = (object) array_merge((array) $filters, (array) $data);
            }
            $this->preprocessData('com_danhmuc.danhhieuphongtang', $data);
        } else {
            $data = (array) $app->getUserState('com_danhmuc.danhhieuphongtang.filter', array());
        }
        return $data;
    }
}
