 ...
 * @since  3.7.0
 */
class DantocModel extends ListModel
{
     ...
     * @throws  \Exception
     */
    public function getTable($type = 'Dantoc', $prefix = 'Joomla\\Component\\Danhmuc\\Administrator\\Table\\', $config = [])
    {
    ...
    protected function getListQuery()
    {
        ...
        $query->from($db->quoteName('danhmuc_dantoc') . ' AS a');
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_danhmuc.dantoc', 'dantoc', ['control' => 'jform', 'load_data' => $loadData]);
    ...
    public function save($data, $pk = null)
    {
        ...
        // $query->insert('danhmuc_dantoc')->columns($columns)->values(implode(',',$values));
        // $db->setQuery($query);
        // return $db->execute();
    ...
    public function getItem($pk = null)
    {
        ...
        $query = $db->getQuery(true);
        $query->select('code, name, level, status')->from('danhmuc_dantoc')->where('code = ' . $pk);
        $db->setQuery($query);
    ...
    public function loadFormData()
    {
        ...
        if ($pk > 0) {
            ...
            if (!$data || !$data->code) {
                $filters = (array) $app->getUserState('com_danhmuc.dantoc.filter');
                $data = (object) array_merge((array) $filters, (array) $data);
            }
            $this->preprocessData('com_danhmuc.dantoc', $data);
        } else {
            $data = (array) $app->getUserState('com_danhmuc.dantoc.filter', array());
        }
        return $data;
    }
}
