 ...
 * @since  3.7.0
 */
class HonnhanModel extends ListModel
{
     ...
     * @throws  \Exception
     */
    public function getTable($type = 'Honnhan', $prefix = 'Joomla\\Component\\Danhmuc\\Administrator\\Table\\', $config = [])
    {
    ...
    protected function getListQuery()
    {
        ...
        $query->from($db->quoteName('danhmuc_tinhtranghonnhan') . ' AS a');
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_danhmuc.honnhan', 'honnhan', ['control' => 'jform', 'load_data' => $loadData]);
    ...
    public function save($data, $pk = null)
    {
        ...
        // $query->insert('danhmuc_tinhtranghonnhan')->columns($columns)->values(implode(',',$values));
        // $db->setQuery($query);
        // return $db->execute();
    ...
    public function getItem($pk = null)
    {
        ...
        $query = $db->getQuery(true);
        $query->select('code, name, level, status')->from('danhmuc_tinhtranghonnhan')->where('code = ' . $pk);
        $db->setQuery($query);
    ...
    public function loadFormData()
    {
        ...
        if ($pk > 0) {
            ...
            if (!$data || !$data->code) {
                $filters = (array) $app->getUserState('com_danhmuc.honnhan.filter');
                $data = (object) array_merge((array) $filters, (array) $data);
            }
            $this->preprocessData('com_danhmuc.honnhan', $data);
        } else {
            $data = (array) $app->getUserState('com_danhmuc.honnhan.filter', array());
        }
        return $data;
    }
}
