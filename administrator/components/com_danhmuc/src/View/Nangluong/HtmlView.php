 ...
 */
namespace Joomla\Component\Danhmuc\Administrator\View\Nangluong;
...
{    
    ...
    public function addToolbar() {
        ToolbarHelper::title(Text::_('COM_DANHMUC_HINHTHUCNANGLUONGS'), 'grid');
        $toolbar = Toolbar::getInstance();
        $canDo = $this->canDo;
        $user  = $this->getCurrentUser();
        // Add "Save" and "Cancel" buttons for the form view edit
        if (Factory::getApplication()->input->getCmd('layout') == 'edit') {
            $toolbar->save2new('nangluong.save2new', 'JTOOLBAR_APPLY');
            $toolbar->save('nangluong.save', 'JTOOLBAR_SAVE');
            // $toolbar->cancel('nangluong.cancel', 'JTOOLBAR_CANCEL');
            $buttonCancel = (new LinkButton('icon'))
                ->url('index.php?option=com_danhmuc&view=nangluong')
                ->text('Cancel')
            ...
            if ($canDo->get('core.create')) {
                $button = (new LinkButton('icon'))
                    ->url('index.php?option=com_danhmuc&view=nangluong&layout=edit')
                    ->text('New')
            ...
            if ($canDo->get('core.delete')) {
                $toolbar->delete('nangluong.delete', 'JTOOLBAR_DELETE')
