 ...
 */
namespace Joomla\Component\Danhmuc\Administrator\View\Danhhieuphongtang;
...
{    
    ...
    public function addToolbar() {
        ToolbarHelper::title(Text::_('COM_DANHMUC_DANHHIEUPHONGTANGS'), 'grid');
        $toolbar = Toolbar::getInstance();
        $canDo = $this->canDo;
        $user  = $this->getCurrentUser();
        // Add "Save" and "Cancel" buttons for the form view edit
        if (Factory::getApplication()->input->getCmd('layout') == 'edit') {
            $toolbar->save2new('danhhieuphongtang.save2new', 'JTOOLBAR_APPLY');
            $toolbar->save('danhhieuphongtang.save', 'JTOOLBAR_SAVE');
            // $toolbar->cancel('danhhieuphongtang.cancel', 'JTOOLBAR_CANCEL');
            $buttonCancel = (new LinkButton('icon'))
                ->url('index.php?option=com_danhmuc&view=danhhieuphongtang')
                ->text('Cancel')
            ...
            if ($canDo->get('core.create')) {
                $button = (new LinkButton('icon'))
                    ->url('index.php?option=com_danhmuc&view=danhhieuphongtang&layout=edit')
                    ->text('New')
            ...
            if ($canDo->get('core.delete')) {
                $toolbar->delete('danhhieuphongtang.delete', 'JTOOLBAR_DELETE')
