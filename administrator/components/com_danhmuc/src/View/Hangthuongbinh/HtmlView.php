 ...
 */
namespace Joomla\Component\Danhmuc\Administrator\View\Hangthuongbinh;
...
{    
    ...
    public function addToolbar() {
        ToolbarHelper::title(Text::_('COM_DANHMUC_HANGTHUONGBINHS'), 'grid');
        $toolbar = Toolbar::getInstance();
        $canDo = $this->canDo;
        $user  = $this->getCurrentUser();
        // Add "Save" and "Cancel" buttons for the form view edit
        if (Factory::getApplication()->input->getCmd('layout') == 'edit') {
            $toolbar->save2new('hangthuongbinh.save2new', 'JTOOLBAR_APPLY');
            $toolbar->save('hangthuongbinh.save', 'JTOOLBAR_SAVE');
            // $toolbar->cancel('hangthuongbinh.cancel', 'JTOOLBAR_CANCEL');
            $buttonCancel = (new LinkButton('icon'))
                ->url('index.php?option=com_danhmuc&view=hangthuongbinh')
                ->text('Cancel')
            ...
            if ($canDo->get('core.create')) {
                $button = (new LinkButton('icon'))
                    ->url('index.php?option=com_danhmuc&view=hangthuongbinh&layout=edit')
                    ->text('New')
            ...
            if ($canDo->get('core.delete')) {
                $toolbar->delete('hangthuongbinh.delete', 'JTOOLBAR_DELETE')
    ...
    public function _pageEdit(){
        $model = $this->getModel('Hangthuongbinh');
