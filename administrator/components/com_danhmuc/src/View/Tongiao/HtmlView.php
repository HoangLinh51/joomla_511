 ...
 */
namespace Joomla\Component\Danhmuc\Administrator\View\Tongiao;
...
{    
    ...
    public function addToolbar() {
        ToolbarHelper::title(Text::_('COM_DANHMUC_TONGIAOS'), 'grid');
        $toolbar = Toolbar::getInstance();
        $canDo = $this->canDo;
        $user  = $this->getCurrentUser();
        // Add "Save" and "Cancel" buttons for the form view edit
        if (Factory::getApplication()->input->getCmd('layout') == 'edit') {
            $toolbar->save2new('tongiao.save2new', 'JTOOLBAR_APPLY');
            $toolbar->save('tongiao.save', 'JTOOLBAR_SAVE');
            // $toolbar->cancel('tongiao.cancel', 'JTOOLBAR_CANCEL');
            $buttonCancel = (new LinkButton('icon'))
                ->url('index.php?option=com_danhmuc&view=tongiao')
                ->text('Cancel')
            ...
            if ($canDo->get('core.create')) {
                $button = (new LinkButton('icon'))
                    ->url('index.php?option=com_danhmuc&view=tongiao&layout=edit')
                    ->text('New')
            ...
            if ($canDo->get('core.delete')) {
                $toolbar->delete('tongiao.delete', 'JTOOLBAR_DELETE')
