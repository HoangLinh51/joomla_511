 ...
 */
namespace Joomla\Component\Danhmuc\Administrator\View\Gioitinh;
...
{    
    ...
    public function addToolbar() {
        ToolbarHelper::title(Text::_('COM_DANHMUC_GIOITINHS'), 'grid');
        $toolbar = Toolbar::getInstance();
        $canDo = $this->canDo;
        $user  = $this->getCurrentUser();
        // Add "Save" and "Cancel" buttons for the form view edit
        if (Factory::getApplication()->input->getCmd('layout') == 'edit') {
            $toolbar->save2new('gioitinh.save2new', 'JTOOLBAR_APPLY');
            $toolbar->save('gioitinh.save', 'JTOOLBAR_SAVE');
            // $toolbar->cancel('gioitinh.cancel', 'JTOOLBAR_CANCEL');
            $buttonCancel = (new LinkButton('icon'))
                ->url('index.php?option=com_danhmuc&view=gioitinh')
                ->text('Cancel')
            ...
            if ($canDo->get('core.create')) {
                $button = (new LinkButton('icon'))
                    ->url('index.php?option=com_danhmuc&view=gioitinh&layout=edit')
                    ->text('New')
            ...
            if ($canDo->get('core.delete')) {
                $toolbar->delete('gioitinh.delete', 'JTOOLBAR_DELETE')
