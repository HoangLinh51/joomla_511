 ...
 */
namespace Joomla\Component\Danhmuc\Administrator\View\Honnhan;
...
{    
    ...
    public function addToolbar() {
        ToolbarHelper::title(Text::_('COM_DANHMUC_TINHTRANGHONNHANS'), 'grid');
        $toolbar = Toolbar::getInstance();
        $canDo = $this->canDo;
        $user  = $this->getCurrentUser();
        // Add "Save" and "Cancel" buttons for the form view edit
        if (Factory::getApplication()->input->getCmd('layout') == 'edit') {
            $toolbar->save2new('honnhan.save2new', 'JTOOLBAR_APPLY');
            $toolbar->save('honnhan.save', 'JTOOLBAR_SAVE');
            // $toolbar->cancel('honnhan.cancel', 'JTOOLBAR_CANCEL');
            $buttonCancel = (new LinkButton('icon'))
                ->url('index.php?option=com_danhmuc&view=honnhan')
                ->text('Cancel')
            ...
            if ($canDo->get('core.create')) {
                $button = (new LinkButton('icon'))
                    ->url('index.php?option=com_danhmuc&view=honnhan&layout=edit')
                    ->text('New')
            ...
            if ($canDo->get('core.delete')) {
                $toolbar->delete('honnhan.delete', 'JTOOLBAR_DELETE')
