 ...
 */
namespace Joomla\Component\Danhmuc\Administrator\View\Doituonghuongcs;
...
{    
    ...
    public function addToolbar() {
        ToolbarHelper::title(Text::_('COM_DANHMUC_DOITUONGHUONGCHINHSACHS'), 'grid');
        $toolbar = Toolbar::getInstance();
        $canDo = $this->canDo;
        $user  = $this->getCurrentUser();
        // Add "Save" and "Cancel" buttons for the form view edit
        if (Factory::getApplication()->input->getCmd('layout') == 'edit') {
            $toolbar->save2new('doituonghuongcs.save2new', 'JTOOLBAR_APPLY');
            $toolbar->save('doituonghuongcs.save', 'JTOOLBAR_SAVE');
            // $toolbar->cancel('doituonghuongcs.cancel', 'JTOOLBAR_CANCEL');
            $buttonCancel = (new LinkButton('icon'))
                ->url('index.php?option=com_danhmuc&view=doituonghuongcs')
                ->text('Cancel')
            ...
            if ($canDo->get('core.create')) {
                $button = (new LinkButton('icon'))
                    ->url('index.php?option=com_danhmuc&view=doituonghuongcs&layout=edit')
                    ->text('New')
            ...
            if ($canDo->get('core.delete')) {
                $toolbar->delete('doituonghuongcs.delete', 'JTOOLBAR_DELETE')
    ...
    public function _pageEdit(){
        $model = $this->getModel('Doituonghuongcs');
