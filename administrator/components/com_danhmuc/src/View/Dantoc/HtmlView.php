 ...
 */
namespace Joomla\Component\Danhmuc\Administrator\View\Dantoc;
...
{    
    ...
    public function addToolbar() {
        ToolbarHelper::title(Text::_('COM_DANHMUC_DANTOCS'), 'grid');
        $toolbar = Toolbar::getInstance();
        $canDo = $this->canDo;
        $user  = $this->getCurrentUser();
        // Add "Save" and "Cancel" buttons for the form view edit
        if (Factory::getApplication()->input->getCmd('layout') == 'edit') {
            $toolbar->save2new('dantoc.save2new', 'JTOOLBAR_APPLY');
            $toolbar->save('dantoc.save', 'JTOOLBAR_SAVE');
            // $toolbar->cancel('dantoc.cancel', 'JTOOLBAR_CANCEL');
            $buttonCancel = (new LinkButton('icon'))
                ->url('index.php?option=com_danhmuc&view=dantoc')
                ->text('Cancel')
            ...
            if ($canDo->get('core.create')) {
                $button = (new LinkButton('icon'))
                    ->url('index.php?option=com_danhmuc&view=dantoc&layout=edit')
                    ->text('New')
            ...
            if ($canDo->get('core.delete')) {
                $toolbar->delete('dantoc.delete', 'JTOOLBAR_DELETE')
