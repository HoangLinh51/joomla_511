 ...
 */
namespace Joomla\Component\Dmluong\Administrator\View\Bangluong;
...
{    
    ...
    public function display($tpl = null) {
        ...
        $this->canDo         = HelperContentHelper::getActions('com_dmluong');
    ...
    public function addToolbar() {
        ToolbarHelper::title(Text::_('COM_DMLUONG_BANGLUONG'), 'grid');
        $toolbar = Toolbar::getInstance();
        $canDo = $this->canDo;
        $user  = $this->getCurrentUser();
        // Add "Save" and "Cancel" buttons for the form view edit
        if (Factory::getApplication()->input->getCmd('layout') == 'edit') {
            $toolbar->save2new('bangluong.save2new', 'JTOOLBAR_APPLY');
            $toolbar->save('bangluong.save', 'JTOOLBAR_SAVE');
            // $toolbar->cancel('bangluong.cancel', 'JTOOLBAR_CANCEL');
            $buttonCancel = (new LinkButton('icon'))
                ->url('index.php?option=com_dmluong&view=bangluong')
                ->text('Cancel')
            ...
            if ($canDo->get('core.create')) {
                $button = (new LinkButton('icon'))
                    ->url('index.php?option=com_dmluong&view=bangluong&layout=edit')
                    ->text('New')
            ...
            if ($canDo->get('core.delete')) {
                $toolbar->delete('bangluong.delete', 'JTOOLBAR_DELETE')
