...
class UserModel extends AdminModel
{
    ...
    public function getDonviId(){
        $usersId = (int) $this->getState('user.id');
        $db = Factory::getDbo();
		$query = 'SELECT id_donvi FROM core_user_donvi 
				WHERE id_user ='.$db->quote($usersId)
		;
		$db->setQuery($query);   
        $row = $db->loadResult();   
        return $row;
    ...
    public function getDonviQuanlyId(){
        $usersId = (int) $this->getState('user.id');
        $db = Factory::getDbo();
		$query = 'SELECT iddonvi_quanly FROM core_user_donvi 
				WHERE id_user ='.$db->quote($usersId)
		;
		$db->setQuery($query);   
        $row = $db->loadResult();   
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_decentralization.user', 'user', ['control' => 'jform', 'load_data' => $loadData]);
         ...
         */
        if (!$user->authorise('core.manage', 'com_decentralization') && (int) $user->id === $userId) {
            if (!ComponentHelper::getParams('com_decentralization')->get('change_login_name')) {
    ...
    protected function loadFormData()
    {
        ...
        $data = Factory::getApplication()->getUserState('com_decentralization.edit.user.data', []);

        if (empty($data)) {
            $data = $this->getItem();
        }

        $this->preprocessData('com_decentralization.profile', $data, 'user');
	 ...
	 * @return mixed
	 */
	public static function delete_User_Donvi($id_user){
		$db= Factory::getDBO();
        $query = $db->getQuery(true);
		$query = 'DELETE FROM core_user_donvi WHERE id_user IN ('.$id_user.')';
        $db->setQuery($query);
        if(!$db->execute()){
            return false;
        }
    ...
    public function save($data)
    {
        ...
        $id_donvi	=	(int)$data['id_donvi_hidden'];
        if ((int)$data['id_donviquanly_hidden'] > 0) {
			$iddonvi_quanly		=	(int)$data['id_donviquanly_hidden'];
		}else {
        ...
        if ((int)$id_donvi > 0 || (int)$iddonvi_quanly > 0) {
            ...
            $object_core_user_donvi->id_user	=	$user->id;
            $object_core_user_donvi->iddonvi_quanly	=	$iddonvi_quanly;
            self::delete_User_Donvi($user->id);
