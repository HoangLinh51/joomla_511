...
class UsersController extends AdminController
{
     ...
     * @since  1.6
     */
    protected $text_prefix = 'COM_DECENTRALIZATION_USERS';
    ...
    public function changeBlock()
    {
        ...
        if (empty($ids)) {
            ...
            if (!$model->block($ids, $value)) {
                ...
                if ($value == 1) {
                    $this->setMessage(Text::plural('COM_DECENTRALIZATION_N_USERS_BLOCKED', count($ids)));
                } elseif ($value == 0) {
                    $this->setMessage(Text::plural('COM_DECENTRALIZATION_N_USERS_UNBLOCKED', count($ids)));
                }
            }
        }

        $this->setRedirect('index.php?option=com_decentralization&view=users');
    }
    ...
    public function tree_data_json(){
       // Send json mime type.
        ...
        DecentralizationHelper::Dept($inArray, $tree_dept);
        unset($inArray);
        header('Content-type: application/json');
		echo json_encode($tree_dept);
		die;
    }
