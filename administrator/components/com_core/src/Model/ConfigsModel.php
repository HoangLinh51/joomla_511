...
class ConfigsModel extends ListModel{
    ...
    public function listConfigById(){
		$id = (int) $this->getState('filter.id');
        $db = Factory::getDbo();
		$query = $db->getQuery(true);
		$query->select(array('a.path','a.id','a.title','b.value', 'b.config_field_id','a.type'))->from('core_config_field AS a');
		if ((int)$id > 0) {			
			$path = $this->getPathById($id);
			$query->where('a.path LIKE '.$db->quote($path.'/%'));
		}
