...
class Menu extends Nested
{
    ...
    {
        parent::__construct('core_menu', 'id', $db);
	...
	public function addRoot() {
		$db = Factory::getDbo ();
        $sql = $db->getQuery();
		$sql = 'INSERT INTO core_menu' . ' SET id = '. $db->quote ( '1' )
		.', parent_id = 0'
				. ', lft = 0'
						. ', rgt = 1'
								. ', level = 0, is_system = 0, published = 1,menu_type=1'
										. ', name = '. $db->quote ( 'Trang chủ' )
										. ', link = '.$db->quote( 'index.php' );
										$db->setQuery ( $sql );
                                        $db->query ();
										// return $db->execute ();
										return $db->insertid ();
